...
-这一阶段默认走 `full_image_ppt` 主线：先确认 full 图、speaker notes 和 template text lock，再通过 `produce prepare -> produce assemble -> produce verify` 组装套模板后的图片型 PPTX 并完成渲染 QA。正文区默认不承诺对象级可编辑；如果明确需要正文文字/对象可编辑，应单独启用 Legacy/Advanced editable rebuild。
...
第一阶段支持可审查的 prompt-first 大模型辅助流程：
...
| Asset Gate | 默认 `full_image_ppt` 是否只消费已批准 full 图；Legacy/Advanced 是否记录图片资产来源和影响 | 默认模式不把正文 full 图强制原生重建；legacy 无必要图片必须改为原生重建 |
...
| Legacy/Advanced Editable Layer Gate | 仅在显式启用 editable rebuild 时，主标题、正文、关键数字、图表标签、页脚、SO WHAT 是否可编辑 | 主要信息图片化即失败 |
| Legacy/Advanced Visual Semantics Gate | 仅在 editable rebuild 中，图表语义、曲线、面板系统、底色、层级和视觉重心是否忠实蓝图 | 不能用“可编辑”解释视觉降级 |

关键原则：默认 `full_image_ppt` 不要求正文区主要文字可编辑，但必须如实声明 `body_content_editable=false`，并守住模板文字层、speaker notes、渲染比对、strict QA 和依赖 freshness。Legacy/Advanced editable rebuild 才要求结构可编辑与视觉还原同时成立。
