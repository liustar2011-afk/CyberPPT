...
2. 必须实际展示 8 张独立完整的 16:9 图片。优先使用内置样张 `assets/palette-samples/palette-01.png` 到 `palette-08.png`；如果重新生成，也必须交付 8 张真实图片。
3. 不得只生成 Markdown、表格、文本列表、推荐理由或 `stage2_style_options.md` 作为风格确认物。
4. 默认必须使用固定 8 种 CyberPPT 视觉风格；除非用户明确要求替换，否则不得改成临场扩展风格。
5. 选定风格后，必须声明并锁定风格编号、名称、色板、网格、标题层级、图表语言、页眉页脚和信息密度规则。
6. 逐页蓝图必须使用第一步确认的页面信息密度和组件清单，不得把高密度计划降级成宽松卡片、泛化图表或稀疏大字报。
7. 蓝图中的文字和数字只作为构图占位，最终 PPT 文本、数据、表格值、图表值和来源必须从证据表重建。

### 第二步工作顺序

1. 展示 8 张固定样张图片，并在图片外列出编号、名称、色板、语气、优势和风险。
2. 等待用户选择或要求替换风格。
...
- 风格样张确认前必须实际展示 8 张独立 16:9 图片。优先使用内置样张 `assets/palette-samples/palette-01.png` 到 `palette-08.png`，或生成新的 8 张真实图片。
- 不得只生成 Markdown、文本列表、表格、样式说明或 `stage2_style_options.md` 作为第二阶段交付；这些只能作为图片旁的辅助说明，不能替代图片。
