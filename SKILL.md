...
| 2. 蓝图 | 8 种视觉风格、选定风格、覆盖全部请求/计划页数的逐页正文内容区 ImageGen 蓝图 | 第二次确认：用户批准视觉方向和全部页面正文区蓝图 | `references/visual-system.md` |
| 3. 混合还原 PPTX | 默认采用 `dual_image_editable_overlay`（无文字底图 + 主要文字可编辑）；仅当用户明确要求背景图表/表格/箭头/图标等对象级可编辑时，升级到 `native_rebuild` | 最终确认：用户批准可编辑 PPT | `references/ppt-production.md`, `references/quality-assurance.md`, `references/dual-image-editable-overlay.md` |
