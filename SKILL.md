...
第一阶段可以使用仓库内置的 prompt-first 模型流程提升材料分析、主线比较、页面设计和业务稿质量。模型生成不是确认结果，任何模型候选都不得自动批准；prompt 必须作为 reviewable prompt 保留并可人工修改。

```bash
python3 -m cyberppt phase1 prepare <project> --gate source_analysis --input <source_extract.md>
# 检查并可直接编辑 workbench/stages/01-analysis/model-runs/<gate>/prompts/<gate>_prompt.md
...
`phase1 prepare` 只生成可审查 prompt，不调用模型；`phase1 generate` 消费用户编辑后的 prompt，并保存原始模型响应、候选稿、grounding QA、模型信息和哈希；`phase1 critique` 只生成批评清单，不改写候选稿；`phase1 stage` 通过确定性校验后才进入既有人工确认门。五道第一阶段门必须逐门执行，不能越过上游批准记录。
