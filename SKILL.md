...
用户只需要用日常语言表达意图，例如"根据这个 Word 生成 PPT 脚本""继续""第 8 页重新写"，Agent 按 `.agents/skills/cyberppt-script-workflow/SKILL.md` 的路由规则自动进入对应阶段；不得要求用户先选择内部阶段名称、Skill 名称或 CLI 命令。
...
1. **交流目标**：先运行 `python -m cyberppt prepare-communication-strategy <project>` 并分析其 `source_outline` 与 `decision_evidence`。基于源材料提出一个忠于原稿的交流目标方向，说明具体受众、使用场景、希望受众理解或相信什么、希望受众采取什么行动及对应 `source unit_id` 依据；不得提供多个选项，不得把用户目标、作者推断或泛化措辞升级为源材料事实、源材料判断或页面主结论。用户目标只作为受众、用途或交付约束。不得直接向用户抛出受众、场景、目标行动等空白问题；用户只需选择、修改或补充建议，收到输入后再确定表达策略。该命令只向当前执行代理输出写作输入，不写沟通策略 JSON、审批、哈希、回执、attempt、manifest 或 ledger。
...
标题、副标题、Logo、页码、页脚和公共模板元素属于模板文字层，不属于正文区 ImageGen full 图的自由推断对象。生图 prompt 必须继续禁止画入这些元素；放开限制会造成坐标漂移、重复标题、模板层碰撞和不受控装饰污染。
...
缺少模板文字层 truth 时必须停在 `metadata_required`，输出缺料清单、建议的 `template_text_lock` scaffold、已接收 full 图路径、不能继续的下游步骤和恢复命令。不得继续执行 `template_image_ppt_export`，不得把 OCR、文件名、图片顶部文字或人工目测结果静默写成标题。
...
套模板后发现正文区问题，必须回到对应页的 full 图或脚本锁定返工，不得只在最终模板 PPT 中手工修补。正文区问题包括但不限于：full 图构图错误、标题误画入正文区、文字污染、图文错位、正文区缩放或裁切错误。
