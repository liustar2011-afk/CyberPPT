...
CyberPPT 默认生产交付模式是 `full_image_ppt`。正文内容由经审批的 full 图承载；标题、副标题、Logo、页码、页脚和公共模板元素由 PowerPoint 模板层生成。正文区主要文字通常不可编辑，交付必须如实记录 `body_content_editable=false`、`template_text_editable=true` 和 `speaker_notes_required=true`。

项目可显式设置 `production_mode: editable_text_three_image`，在共享第一阶段脚本、模板文字锁定和 speaker notes 审批之后，走三图转 PPT 分支。该分支要求 FULL、无文字 BACKGROUND、TEXT、OCR 和 registration 输入，调用 `vendor/three-image-to-ppt` 生成并校验逐行 `page.json`；BACKGROUND 进入模板正文区，每条视觉行生成一个原生可编辑文本框。vendor `review` 必须经过人工复核，`failed` 不得装配；OCR 不能推断标题或副标题。
...
三图分支在 speaker notes 审批后使用：

```bash
python3 -m cyberppt produce editable-text <project> --pages <range>
python3 -m cyberppt produce assemble <project> --pages <range>
python3 -m cyberppt produce verify <project> --pages <range>
```

`produce editable-text` 只生成项目内 vendor 批处理工件并停在必要的复核门；`produce assemble` 只消费通过的 `qa.json`、`page.json` 和三图哈希，不重新生成图像；最终交付 manifest 的 `delivery_mode` 为 `editable_text_three_image`，并记录 `body_content_editable=true`。
