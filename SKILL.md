...
| 2. 蓝图与双图资产 | 8 种视觉风格、选定风格、逐页正文区 ImageGen 蓝图、脚本锁定记录、ImageGen full 图、no-text background 派生计划、`page_image_pairs.json` | 第二次确认：用户批准视觉方向、全部页面正文区蓝图和进入双图转换的脚本/图像资产 | `references/visual-system.md` |
| 3. 混合还原 PPTX | 默认采用 `dual_image_editable_overlay`（无文字底图 + 主要文字可编辑）生成正文区可编辑层，再通过 `template_rebuild` 套入模板内容区；仅当用户明确要求背景图表/表格/箭头/图标等对象级可编辑时，升级到 `native_rebuild` | 最终确认：用户批准套模板后的可编辑 PPT | `references/ppt-production.md`, `references/quality-assurance.md`, `references/dual-image-editable-overlay.md` |
...
`脚本锁定 -> 正文区 ImageGen full 图 -> no-text background -> dual_image_editable_overlay -> template_rebuild -> 渲染 QA -> 交付`

生成 PPT 必须经过双图环节。任何 CyberPPT 正式 PPTX 页面都不得绕过 `正文区 ImageGen full 图 -> no-text background -> dual_image_editable_overlay`，不得从脚本、Markdown、大纲、SVG、HTML、PptxGenJS、python-pptx、PowerPoint 或其他本地绘图方式直接手画成最终 PPT。即使页面看起来简单，也必须先形成 full/background 双图资产、`page_image_pairs.json` 和 overlay 阶段工件，再进入 `template_rebuild`；否则只能标记为流程失败或手工草稿，不能称为 CyberPPT 合格交付物。

含义如下：

1. **脚本锁定**：逐页保存并确认脚本、内容锁定、模板文字层锁定、视觉锁定和必要的 ImageGen prompt。脚本是后续双图和可编辑文字层的 truth，不得把 ImageGen 图中文字或 OCR 当作最终内容来源。
2. **正文区 ImageGen full 图**：按已确认脚本和视觉锁定生成正文内容区 full 图；full 图不包含标题、副标题、Logo、页码、页脚、蓝线或公共模板元素。
3. **no-text background**：从 approved full 派生无文字 background，保持同一构图；不得重新文生图生成同主题新图替代 background。
4. **dual_image_editable_overlay**：用 background 作为正文区视觉底图，用脚本/semantic_plan 生成主要文字、数字和 SO WHAT 的可编辑文本层，并执行 background no-text scan、text-content QA、layout QA、semantic typography QA、Office text box fit 和 visual QA。
5. **template_rebuild**：把正文区双图结果套入模板内容区，由模板层生成标题、副标题、Logo、页脚、页码、蓝线和公共元素。未经 `template_rebuild` 套入模板内容区的 overlay PPTX 只能作为中间产物，不得作为 CyberPPT 最终交付物。
6. **渲染 QA 与交付**：最终验收对象是套模板后的 PPTX 渲染结果，不是未套模板的 overlay 页、XML 结构检查或单独的图片资产。

允许用户手工指定走到哪一步，例如只到脚本、只到 ImageGen full、只到双图、只到 overlay、只到 template rebuild 或只做 QA。手工停点必须记录当前停点、已完成工件、未执行后续步骤和恢复命令；不得把停点产物冒充最终交付物。

## 模板文字层 truth 与中途接入

标题、副标题、Logo、页码、页脚和公共模板元素属于模板文字层，不属于正文区 ImageGen full/background，也不属于正文区 overlay 的自由推断对象。生图 prompt 必须继续禁止画入这些元素；放开限制会造成坐标漂移、重复标题、模板层碰撞和不受控装饰污染。

模板文字层必须有独立 truth。默认从脚本锁定阶段生成 `template_text_lock`，至少记录每页 `page`、`title`、`subtitle`、`section`、`template_variant`、`page_badge_enabled`、`footer_enabled`、`source`、`approved`、`depends_on` 和 `resume_command`。该锁定文件必须登记进 `artifact-ledger.json`，并作为 `template_rebuild` 的必需输入。

不得从 full 图、background 图或 OCR 猜测标题和副标题。OCR 只能用于发现污染或碰撞，例如 full/background 中误画了标题、副标题、页码或 Logo；发现后应标记为正文区图像污染，要求重新裁切、遮罩、去字或重生正文区资产，而不是把这些像素识别成可靠标题层。

中途接入双图时必须提供 `template_text_lock` 或等价标题层 metadata。等价 metadata 必须能覆盖 `template_rebuild` 所需的标题、副标题、模板开关和来源追踪，并登记到 ledger；不能只提供 full/background 两张图后让后续脚本猜标题。

缺少模板文字层 truth 时必须停在 `metadata_required`，输出缺料清单、建议的 `template_text_lock` scaffold、已接收双图路径、不能继续的下游步骤和恢复命令。不得继续执行 `template_rebuild`，不得把 OCR、文件名、图片顶部文字或人工目测结果静默写成标题。
...
| 蓝图与双图资产 | `workbench/stages/02-blueprint-dual-image/` | 风格锁定、`slide_content_lock`、`template_text_lock`、ImageGen prompt、full 图、background 派生记录、`page_image_pairs.json` |
| Overlay 转换 | `workbench/stages/03-overlay/` | semantic_plan、text mapping、office_textbox_fit、overlay PPTX、overlay render、局部 QA |
| 套模板 | `workbench/stages/04-template-rebuild/` | template PPTX、template_rebuild_readiness、source_capture、template-normalized reference、模板层 QA |
| QA 与交付 | `workbench/stages/05-qa-delivery/` | visual_qa_gate、slide_manifest、side-by-side、局部裁图、最终 deck、交付说明 |

项目必须维护 `artifact-ledger.json`。每个成果物必须记录 `stage`、`page`、`path`、`status`、`depends_on`、`supersedes` 和 `resume_command`；能计算 hash 时还必须记录 SHA-256。`depends_on` 指向上游成果物，`supersedes` 指向被本次返工替代的旧成果物。这样套模板后发现问题时，可以沿 `depends_on` 反查到脚本、full/background、overlay 或模板层中真正需要修改的来源。
...
- 第三阶段默认交付模式为 `dual_image_editable_overlay`；启用或默认执行 `dual_image_editable_overlay` 时，还必须读取 `references/dual-image-editable-overlay.md`，并把无文字底图、可编辑文字层、prompt 编译、background no-text scan、text-content QA、layout QA 和 production readiness 要求转成执行清单。

## 默认页面结构策略（页眉页脚 / 页码徽章 / 保密声明）

全仓库默认行为（适用于所有项目，除非用户在当前项目中另行明确要求）：

- 内容页默认**不设左上角页码/章节徽章**。不使用 Typography Scale `T1` 承载页码或章节编号徽章；`T1` 层级定义仍保留在本文件和 reference 中，仅供用户明确要求启用页码徽章时使用。
- 内容页默认**不设独立页脚区**。原计划放入页脚的来源说明、证据 ID、口径、页码等内容，一律改为随对应证据块/图表/表格就近以 `T14` 小字标注在内容区内部，不得因页脚关闭而整体省略。
- 任何页面默认**不得出现"内部资料，注意保密""内部文件，请勿外传""机密"等保密声明类文字**，除非用户明确要求加入。
- 以上默认适用于封面、内容页和收尾页；页眉仍可按需保留章节名（不含页码徽章）。

条件式 QA 执行规则（不删除检查机制本身，只改为按开关跳过，机制细节仍见 `quality-assurance.md` / `ppt-production.md` 原文）：

- 当某页 `header_footer_system.page_badge_enabled = false`（默认值）时，本文件及 reference 中全部与"页码徽章"相关的 P0 测量、容器边界、可编辑层检查项一律判定为 `not_applicable`，不计入失败，也不得因该页没有页码徽章而判定失败。
- 当某页 `header_footer_system.footer_enabled = false`（默认值）时，全部与"页脚"相关的 P0/P1 测量、容器边界、可编辑层、来源说明位置检查同样判定为 `not_applicable`。
...
## 第二步：8 张固定样张 + 逐页正文区 ImageGen 蓝图

第二步不是文字风格选择，也不是只写一份样式说明。它分成两个子阶段：先实际展示 8 张固定 CyberPPT 视觉样张供用户选择，再按选定风格和第一步确认的页面密度生成完整页数的逐页正文内容区 ImageGen 蓝图。正文区蓝图是主线；页面标题、副标题、Logo、页脚、页码、蓝线和公共模板元素不进入 ImageGen 蓝图画面，由模板/母版/可编辑文字层生成。
...
8. 选定风格后，必须声明并锁定风格编号、名称、色板、正文区网格、正文区层级、图表语言、模板层元素和信息密度规则。
9. 逐页正文区蓝图必须使用第一步确认的页面信息密度和组件清单，不得把高密度计划降级成宽松卡片、泛化图表或稀疏大字报。
...
12. 逐页正文区 ImageGen 蓝图必须使用有效目标语言。有效目标语言 = 默认目标交付语言 + 已登记的页级、章节级或组件级语言覆盖。蓝图正文区中的模块标题、图表标签、图例、轴标签、注释、来源、SO WHAT 和其他正文区可见文字占位都必须使用对应范围的有效目标语言；页面标题、副标题、页脚、页码、Logo、蓝线和公共模板元素不得画入蓝图。
...
- 不得在逐页正文区蓝图中画入页面标题、副标题、Logo、页脚、页码、蓝线、母版红线、完整 PPT 外框或任何企业公共模板元素；这些必须由模板/母版/可编辑文字层生成。
...
第三阶段默认交付模式为 `dual_image_editable_overlay`。该模式允许一张无文字正文内容区背景图，但必须声明 `delivery_mode=dual_image_editable_overlay`，并通过 background no-text scan、text-content QA、layout QA 和 visual QA。只有当用户明确要求图表、表格、箭头、图标或背景对象可编辑时，才升级到 `native_rebuild`。

`native_rebuild` 只是在双图 overlay 之后对特定背景对象做对象级可编辑增强，不是绕过双图的替代主线。不得因为“页面简单”“时间紧”“已有脚本”“可直接用形状画出来”而跳过 full/background 双图和 overlay；直接手画 PPT、直接从脚本生成最终 PPTX、直接套模板生成正文页，均视为正式流程失败。

已批准的第二阶段正文区 ImageGen 蓝图默认晋升为 `full` 候选图，不得在第三阶段无理由从零重新 text-to-image 生成另一张 full 图。只有当蓝图未达到最终交付质量、用户要求重做、或 no-text background 派生失败且需要 image-to-image 修复时，才允许基于已批准蓝图做定向重绘，并必须记录原因。

双图生成前必须先用 `scripts/dual_image_overlay/deliverable_prompt.py` 把项目脚本编译成最终交付 prompt，用于补充或修复 full 候选，而不是默认取代已批准蓝图。该 prompt 必须使用项目自身视觉锁定，不得用外部仓库 style preset 覆盖项目风格；生成图必须面向最终客户交付，不得画入证据编号、来源编号、caveat、脚注、口径说明、标题占位条、页码、Logo、页脚、公共元素或调试标记。标题、副标题和公共元素由 PPT 模板/母版与可编辑文字层生成。

当用户要求“完整图定位文字、无字图做底稿”“底图不编辑、主要文字可编辑”“快速高保真交付”或未明确要求背景对象级可编辑时，一律走默认 `dual_image_editable_overlay`。不得把默认路径降回 `native_rebuild`，除非用户明确要求可编辑图表、可编辑表格、可编辑箭头、可编辑图标或可编辑背景对象。

### 手工停点与阶段恢复

用户可以明确要求停在任一子步骤：`script_locked`、`full_generated`、`background_generated`、`overlay_generated`、`template_rebuilt`、`qa_rendered` 或 `deliverable_ready`。执行者必须尊重该停点，不得擅自继续到下一步。

每次停点输出必须包含：

- 当前停点名称；
- 已完成工件路径，包括脚本、prompt、full、background、`page_image_pairs.json`、semantic_plan、overlay PPTX、template PPTX、渲染图和 QA 文件中已存在的部分；
- 未执行的后续步骤；
- 恢复命令或恢复入口；
- 明确标注该停点产物是否可交付。

除 `deliverable_ready` 外，所有停点产物都只是阶段工件，不得称为最终 PPT。

### 套模板后的返工回路

套模板后发现正文区问题，必须回到对应页的双图转换工件返工，不得只在最终模板 PPT 中手工修补。正文区问题包括但不限于：full 图构图错误、background 去字不干净、文字归属错误、文本框溢出、字号/字重不一致、行距不当、语义标题归属错误、图文错位、正文区缩放或裁切错误。

返工规则：

1. 模板阶段可以暴露正文区问题，但不能成为正文区修补点。
2. 只重跑受影响页面和受影响阶段；不得无理由重跑全 deck。
3. 如果问题来自脚本或内容 truth，回到脚本锁定；如果问题来自 full/background，回到双图生成；如果问题来自文本映射或 Office 排版，回到 `dual_image_editable_overlay`。
4. 重新生成正文区 overlay 后必须重新执行 `template_rebuild`，并重新渲染套模板后的页面。
5. 最终 deck 只能合并通过 QA 的最新 template page，不得混用旧 overlay、旧 template page 或旧截图。
...
- `P0`：标题、主图、SO WHAT、关键数字、核心面板、用户指出区域，以及页脚（仅当 `footer_enabled=true` 时纳入，默认关闭时 `not_applicable`）。默认容差 3px，最大 6px；超差即 Critical。
...
- P0/P1 不得被降级为 P2；主图、标题、SO WHAT、关键数字、核心面板和用户指出区域被标成 P2，视为失败；页脚仅当页面启用页脚时适用此规则。
...
| T1 | 页码/章节徽章（默认不启用） | 左上角页码徽章、章节编号徽章 | 14-18pt |
...
- 视觉系统确认后，必须先为完整页数生成逐页正文内容区 ImageGen 蓝图，再进入混合还原 PPTX。蓝图只用于正文区构图、层级、密度和图表语言；标题、副标题、Logo、页脚、页码、蓝线和公共模板元素由模板/母版/可编辑文字层生成。
- 逐页正文区蓝图阶段必须沿用已选风格，不得重新发散；每页蓝图提示词和检查记录都要包含锁定风格编号、色板、正文区网格、信息密度和图表语言。
- 除非用户明确要求跳过 ImageGen，逐页正文区蓝图必须由 ImageGen 生成 bitmap 图片；不得用 PptxGenJS、python-pptx、HTML、CSS、SVG、canvas、Pillow、matplotlib、PowerPoint 或任何本地绘图脚本直接绘制蓝图。
...
- 用 `generate_stage2_blueprints.py`、PptxGenJS、python-pptx、HTML、CSS、SVG、canvas、Pillow、matplotlib 或 PowerPoint 直接生成逐页蓝图，属于第二阶段失败；这些工具不能替代 ImageGen。主线 prompt 编译入口是 `scripts/body_blueprint_prompt.py`，它只能组织正文区 ImageGen prompt 和 manifest，不能替代 ImageGen 生图。
