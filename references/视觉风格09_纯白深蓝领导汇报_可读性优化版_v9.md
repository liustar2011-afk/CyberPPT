...
Understand the source before rewriting it. Preserve the source's core meaning, logical relationship and claim strength, but do not preserve its sentence structure or wording volume mechanically.

Rewrite the supplied source into concise, conclusion-first Chinese presentation text. Moderate semantic clarification and limited low-risk enrichment are allowed when they strengthen an existing argument, clarify a relationship or improve independent readability. Do not use enrichment to create additional topics, frameworks or sections that compete with the source's main logic.

If a page title is supplied in the input, use it as the primary semantic anchor. Do not replace it with a broader, stronger or more generic self-generated headline.

Place rewritten text directly beside, above, below or partially within the related business object, scene or structural carrier, so text and visual material form one local content unit.

Preserve essential business objects, facts, numbers, conditions, scope, responsibility, status and claim strength accurately.

### Readability-first information budget

Readability has priority over textual completeness.

Preserve meaning, not wording volume. Compress aggressively when needed for readability while retaining essential facts, relationships, conditions and conclusions.

Never solve content overflow by shrinking body text, tightening line spacing excessively, or filling every available area. If the page becomes dense, reduce content in this order:

1. remove secondary explanation;
2. merge overlapping or repetitive statements;
3. remove redundant labels and headings;
4. reduce secondary semantic regions;
5. remove low-value scenes, icons or decorative elements.

Only after these reductions should layout density increase.

Prefer:
- one dominant conclusion or judgment;
- compact supporting statements;
- one short local explanation in most cases;
- two short sentences only when the second carries an indispensable fact, condition or implication;
- short bullets only when parallel items genuinely need distinction;
- meaningful phrases rather than isolated keywords;
- fewer, larger and more readable text regions over many small text blocks.

Do not reproduce every sentence of the source merely because it is available.

Do not turn every supporting detail into a visible module.

The page should work as a reading-type PPT and remain understandable without oral explanation, but independent readability does not require full prose reproduction.

A page should feel clear at first glance, not merely complete when inspected closely.

Use semantic understanding to improve the argument, then edit ruthlessly for the screen.
...
Use only the number of realistic scenes that materially improve understanding. For most pages, prefer one or two meaningful scenes. Add more only when genuinely different real-world contexts, stakeholders or evidence need visual separation.

Prefer one strong scene integrated with the information structure over a collage of loosely related images. Do not create additional semantic regions merely to justify more scenes.
...
Treat each archetype as a spatial grammar rather than a content quota. Suggested region or stage counts are flexible; never invent additional content merely to fill the structure.
...
Do not create a bottom band when no genuine cross-cutting semantics exist. The bottom band is not a mandatory summary zone. Do not synthesize generic “技术、标准、安全、运营、政策、机制、保障”等 support categories merely to fill the lower area or make the page look complete.
...
Do not convert every text point into an icon-text pair. If the concept is already clear through text, structure or scene, omit the icon.
...
1. semantic fidelity and logical clarity;
2. readability at intended presentation size;
3. one dominant focus and strong hierarchy;
4. selective semantic enrichment;
5. scene integration and visual refinement.
...
- concise, comfortably readable body text;
- integrated realistic scenes where useful;
...
Never reduce text to microtype in order to preserve secondary detail.

If all content cannot remain comfortably readable at the intended presentation size, simplify the content before shrinking the typography.

Favor fewer, larger text blocks over many small explanations.

When content and readability conflict, keep the core judgment, essential facts, numbers, relationships and conditions; compress, merge or omit secondary detail.
...
Expand selectively, then edit ruthlessly. Use model knowledge and semantic reasoning to strengthen the existing argument, but display only the information necessary to make the page clear, convincing and readable.
...
- realistic contextual electric-power scenes used selectively;
...
Final result: a calm, logical, highly readable and presentation-ready executive-report visual system that preserves Style 09's semantic strength while preventing over-expansion, excessive detail, small text and visual crowding.
