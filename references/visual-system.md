...
面向政企领导汇报与演讲辅助：象牙白 #F7F6F0、深蓝 #12355B；专业、克制、大气。视觉身份为 senior leadership briefing / speech-support，不用咨询交付件、学术论文、软件产品宣传或传统流程信息图的默认审美。

风格只约束气质（色、材质、线条、字形、留白与克制度），不预设场景、插画、照片、图标或三维物件。内容决定构图与媒介：先抓核心判断与业务关系，再定位置、尺度、分组和阅读路径；一条主叙事路径、一个主视觉中心，不强制居中、不做机械均分。页面语义另定载体——抽象主题（治理、权限、标准、知识结构、系统能力等）默认用精细中文排版 + 一处克制深蓝形面或低对比数据纹理；仅当页面必须依赖人物、设备、地点或操作行为时才用场景。

默认不做行业景观、陈旧办公物件字面替代（档案柜、文件夹、牛皮纸、数据库圆柱、机柜等），不做背景图＋前景信息图。关系优先靠层级、位置、尺度、密度与留白表达；必要时才用少量发丝线或低对比标记，不默认画节点链路、箭头光束、架构层或技术面板。

场景是条件性辅助层：能直接解释本页不可替代动作或环境时局部、低对比嵌入；否则省略。禁止为“丰富画面”逐模块配图、风景海报、等权卡片墙、编号步骤卡、四栏咨询排版、SaaS/仪表盘、软件架构图观感、图标墙与等距三维小组件。

人物默认不出镜。确需人的操作/协同时仅远景、小尺度、低对比、侧背或虚化；禁止近景正脸、摆拍与宣传摄影，人物不得抢主视觉或承担核心判断。

视觉优先级：核心判断 → 业务关系与能力形成 → 价值结果 → 匹配媒介的载体 → 必要辅助。气质归风格，媒介归页面语义，二者分责。
