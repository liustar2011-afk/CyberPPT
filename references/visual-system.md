...
面向政企领导汇报与演讲辅助，采用象牙白 #F7F6F0 与深蓝 #12355B。风格只约束气质、材质、色彩、尺度和克制度，不规定页面结构。整体专业、克制、大气、可信，视觉身份为 senior leadership briefing / speech-support。默认采用编辑式业务版面，而不是技术架构图；用统一材质、清晰尺度、文字层级和明确主次承载业务判断。

先识别页面核心判断、业务对象、证据类型及其关系，再决定位置、尺度、分组、阅读路径和视觉载体。内容决定构图，不从预设版式、固定模板、后端构图字段或习惯性布局骨架出发。页面应形成清晰阅读顺序和稳定叙事重心，但重心的数量、位置、方向和载体由页面内容决定，不强制居中、单中心或任何固定空间关系。

优先构建业务结构，而非铺陈行业景观。对于治理、权限、标准、知识结构、系统能力等抽象主题，重点表达业务对象、证据、关系、边界和结果，而非罗列技术名词；当页面涉及资产、能力、流程或服务的形成时，优先表现对象从来源、变化、治理、组合到应用结果的价值形成过程，具体阶段、关系和载体由页面脚本决定，不预设固定路径。优先使用精细中文排版、版面分组、对象叠置、局部放大、尺度对比和留白表达关系。半立体对象只能作为局部语义载体，不得形成圆台、圆盘、卡片堆、模块墙、容器组或设备陈列；禁止用档案柜、文件夹、牛皮纸、数据库圆柱或服务器机柜字面代替抽象概念。

禁止技术架构图化：不得将页面默认表现为软件架构图、数据平台架构图、技术分层图、组件拓扑图或后台模块地图。技术组件仅作为支撑能力，是否出现、出现多少以及采用何种媒介，必须由页面语义决定，不得成为视觉主体。涉及平台、系统或数据服务时，优先表现业务可见的能力输出、使用方式、结果和边界；数据库、索引、缓存、队列、容器等底层组件不作为默认主视觉，只有页面明确要求时才可作为弱化支撑层出现。

优先采用由页面脚本决定的编辑式业务表达：视觉中心、对象关系、阅读方向、输入输出位置和支撑关系均由页面内容决定，不预设中心对象、左右汇聚、单向展开或贯穿轨迹。允许低起伏半立体、浅层空间、纸面/树脂/磨砂材质和柔和接触阴影，但不得形成固定的圆盘、圆台、轨道、节点环或三维模块堆叠；禁止夸张三维、金属高光、玻璃拟态和发光科技装置。风格09可以按页面需要加入轻淡、低对比、局部化的背景图、实景切片或环境纹理，作为承托版面层次和材质的可选支撑层；其透明度、裁切、位置和面积必须服从页面内容，可采用虚化、单色化、边缘渐隐或局部遮罩处理。背景图不得承载核心判断、关键事实或完整叙事，不得形成独立风景海报，不得压过正文、证据和业务结构，也不得被机械地复制到每一页。场景是条件性辅助层，只能作为局部环境或低对比辅助层。

图形应从文本逻辑派生，将输入、输出、依赖、反馈、演进、协同、对比和价值关系转化为空间组织、对象联系和状态变化。优先使用文字尺度、版面分组、对象叠置、局部放大和留白表达关系；线条、箭头、容器和图标不是默认视觉语言，只有页面语义明确要求时才允许使用。服务能力应作为业务对象的输出环境来表现，承载检索、计算、分析、协同或其他页面明确要求的使用结果，不突出底层技术组件清单。不得机械地把每项文字转换成一个图标、卡片或独立模块。文字、图形与业务对象共同组成一体，不得简单处理成“背景图片＋前景信息图”。

除非页面内容本身明确要求，禁止等权卡片墙、编号步骤卡、时间线、中心模块加卫星节点、机械流程模板、四栏咨询排版、SaaS界面、仪表盘布局、软件架构图观感和技术规格图观感。不得默认使用中心对象加左右输入输出、连续箭头、闭环轨道、底部步骤带、多组深蓝卡片或一项内容一个图标。页面应形成由内容决定的层级与权重差异，而不是套用并列模块或固定骨架。保留适度留白，但不得用大面积空白掩盖信息不足；正文信息密集时使用主文、旁注、事实条和细分隔组织，不得擅自压缩关键事实。

人物不是必需元素，默认不出现。仅当页面语义确实依赖人的操作、判断、协同或验证时，才允许少量、远景、小尺度、低对比度人物作为辅助。避免正面人物、近景人物、可识别面部、直视镜头、会议摆拍、握手、人物指向屏幕和企业宣传摄影。人物不得成为页面最醒目的视觉元素，也不得承担核心判断的表达任务。

所有视觉对象和辅助标签必须来自页面已有业务文字，不得补充脚本未提供的服务名称、技术标签、事实、口号或解释性标题。视觉优先级：页面核心判断 → 与判断直接相关的对象、证据和关系 → 业务结果或边界 → 局部环境与辅助元素。场景不是默认主角，图标不是默认载体，半立体质感只用于增强对象层级，不得替代业务关系。气质归风格，媒介归页面语义。
