...
先识别页面核心判断、业务对象、证据类型及其关系，再决定位置、尺度、分组和视觉载体。不得从预设版式、后端构图字段或固定布局骨架出发。页面形成一条清晰的阅读路径和一个主叙事中心，但不强制居中主物体；不同内容应有明确的视觉主次，不做机械均分。

优先采用场景化行业插画、编辑式业务场景或具有行业语义的综合视觉表达，不要求照片式实景。场景可以作为局部主视觉、结构内嵌内容、中景环境，也允许作为虚化、弱化、低对比度的背景图；是否使用及其视觉重量由页面内容决定。场景应帮助建立业务语境或表达页面判断，不得成为无关装饰，也不得压过内容形成风景海报。

图形应从文本逻辑派生，将因果、输入输出、依赖、反馈、演进、对比和价值形成关系转化为空间组织、对象联系、状态变化和作用路径。文字、图形与场景应共同组成一个叙事，不得简单处理成“背景图片＋前景信息图”。允许使用必要的图标、线条、模块和箭头，但不得机械地将每项文字转换为一个图标或独立卡片。

除非页面内容本身明确要求，避免等权卡片墙、编号步骤卡、时间线、中心模块加卫星节点、机械流程模板、四栏咨询排版、SaaS界面和仪表盘式布局。避免两个极端：一是场景过强，形成独立海报；二是场景与插画完全消失，退化为白底框线和图标组成的普通信息图。

人物不是必需元素，默认不出现。仅当页面语义确实依赖人的操作、判断、协同或验证时，才允许少量、远景、小尺度、低对比度人物作为辅助。避免正面人物、近景人物、可识别面部、直视镜头、会议摆拍、握手、人物指向屏幕和企业宣传摄影。
