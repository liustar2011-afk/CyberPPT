...
- 形成清楚的视觉焦点与阅读主线：优先让真实业务对象、连续行业场景、关键动作、结果状态或开放关系场承担主载体。平台、中枢、引擎、中心等抽象业务概念应通过真实业务对象、业务关系、工作场景或结果状态表达其含义，不自动转译为物理展台、圆盘、机械核心、发光装置或其他象征性实体；具体选择由页面语义和 Stage02 结构决定。
...
- 行业场景、设备、工作面、信息流、资料、屏幕、设施和人物可按页面语义作为视觉载体；它们必须解释内容，不作为泛科技装饰。屏幕仅作为业务证据或工作场景组成部分，不生成完整产品 UI、Dashboard 或产品展示界面；人物仅作为辅助尺度、动作或协作线索，不作为主视觉，避免正面人物肖像。抽象主题可使用干净的平面关系场。
...
- 中文字体视觉接近 Microsoft YaHei / 现代无衬线黑体：清晰、中性、稳健、适合政企领导汇报；避免书法体、宋体式装饰、圆体或夸张展示字体。字号层级和版式比例继续服从全局 Typography Scale，不在 Style 09 内另设字号体系。
...
保持纯白底、深蓝主关系、平面2D信息结构和无图标优先。先遵从页面既定的主判断、业务关系和阅读顺序，再用一个清楚的视觉焦点组织场景、对象、文字与证据；主次通过尺度、裁切、留白、对比和自然前后层次建立。平台、中枢、引擎、中心等抽象业务概念通过真实业务对象、业务关系、工作场景或结果状态表达，不自动画成物理展台、圆盘、机械核心、发光装置或悬浮平台。真实场景与真实物体可以保留自然透视和体积，信息模块、文字承载面、关系结构和抽象概念保持平面，不用立体底座、挤出、透视盒或舞台式造型建立层级。锁定中文必须完整清晰，字体视觉接近 Microsoft YaHei 或现代无衬线黑体，并贴近对应对象、动作、接口、边界或结果。多处配图和局部对象可以共存，但必须共享同一业务关系场和阅读主线，不形成独立图片窗、缩略图拼贴或照片墙；不得根据文字条目数量自动生成等宽、等高、等权卡片或模块。屏幕仅作业务证据或工作场景组成部分，不生成完整产品 UI 或 Dashboard；人物仅作辅助尺度、动作或协作线索，不作为主视觉并避免正面人物肖像。连接线和承载面保持克制、清楚、服务关系。图标默认数量为0，不得以图标、徽章或抽象符号替代业务对象、关系或文字。最终效果应像高级领导汇报中的编辑式业务页面：整洁、有焦点、有图文融合，也保留足够的构图自由度。
