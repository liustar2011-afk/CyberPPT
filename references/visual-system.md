...
图形应从文本逻辑派生，将因果、输入输出、依赖、反馈、演进、协同、对比和价值形成关系转化为空间组织、对象联系、状态变化和作用路径。文字、图形与场景应共同组成同一叙事，不得简单处理成“背景图片＋前景信息图”。允许使用必要的标签、线条、模块、箭头和少量图标，但不得机械地将每项文字转换为一个图标、卡片或独立模块。

除非页面内容本身明确要求，避免等权卡片墙、编号步骤卡、时间线、中心模块加卫星节点、机械流程模板、四栏咨询排版、SaaS界面、仪表盘布局、软件架构图观感和技术规格图观感。页面中的信息应有明确主次和视觉权重差异，主叙事统领全页，避免多个彼此割裂的视觉中心和视觉语言。
