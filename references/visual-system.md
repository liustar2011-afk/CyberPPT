...
面向政企领导汇报与演讲辅助，采用象牙白 #F7F6F0 与深蓝 #12355B。风格只约束气质、材质、色彩、尺度和克制度，不预设固定版式。整体专业、克制、大气、可信，视觉身份为 senior leadership briefing / speech-support。审美重点是“编辑式业务结构 + 克制半立体载体”：用统一材质、清晰尺度和明确主次承载业务判断，不采用咨询交付件、学术论文、软件产品宣传或普通流程信息图的默认审美。
...
优先构建业务结构，而非铺陈行业景观。对于治理、权限、标准、知识结构、系统能力等抽象主题，重点表达业务对象、证据、关系、边界和结果，而非罗列技术名词；可用精细中文排版、真实物件、抽象物件或半立体结构承载语义。载体必须形成同一套材质与空间语法，不能变成设备陈列或软件界面拼贴；禁止用档案柜、文件夹、牛皮纸、数据库圆柱或服务器机柜字面代替抽象概念。观众应先感知页面判断和其支撑关系，再理解环境和技术实现。

优先采用“业务对象主导”的编辑式综合视觉表达：由页面脚本决定视觉中心、对象关系、阅读方向、输入输出位置和支撑关系，不预设中心对象、左右汇聚、单向展开或贯穿轨迹。允许使用低起伏半立体、浅层空间、纸面/树脂/磨砂材质和柔和接触阴影，形成可感知的层级与质感；禁止夸张三维、金属高光、玻璃拟态和发光科技装置。场景是条件性辅助层，只能作为局部环境或低对比辅助层，不能占据半页成为独立背景海报。

图形应从文本逻辑派生，将输入、输出、依赖、反馈、演进、协同、对比和价值关系转化为空间组织、对象联系和状态变化。允许使用细线数据流、少量方向标记、层叠面、容器和对象标签，但这些元素的数量、位置和作用必须由页面语义决定，不得机械地把每项文字转换成一个图标、卡片或独立模块。文字、图形与业务对象共同组成一体，不得简单处理成“背景图片＋前景信息图”。

除非页面内容本身明确要求，避免等权卡片墙、编号步骤卡、时间线、中心模块加卫星节点、机械流程模板、四栏咨询排版、SaaS界面、仪表盘布局、软件架构图观感和技术规格图观感。页面应形成由内容决定的层级与权重差异，而不是套用并列模块或固定骨架。保留适度留白，但不得用大面积空白掩盖信息不足；正文信息密集时使用主文、旁注、事实条和细分隔组织，不得擅自压缩关键事实。
...
视觉优先级：页面核心判断 → 与判断直接相关的对象、证据和关系 → 业务结果或边界 → 局部环境与辅助元素。场景不是默认主角，图标不是默认载体，半立体质感只用于增强对象层级，不得替代业务关系。气质归风格，媒介归页面语义。
