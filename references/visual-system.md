...
优先构建业务语境，而非展示行业景观。重点表达业务能力如何形成、协同、运行、支撑决策并产生价值，而非罗列行业设施、技术组件、软件模块或抽象图标。观众应先感知正在发生什么业务活动、哪些对象正在相互作用、能力如何形成和发挥作用、最终产生什么结果或价值，其次再理解行业环境、技术实现、组织角色和支撑设施。

优先采用场景化行业彩色插画、编辑式业务场景或具有行业语义的综合视觉表达，不要求照片式实景。场景仅用于建立业务语境，其形式和视觉重量由页面内容决定，可作为局部主视觉、结构内嵌内容、中景环境，也允许作为虚化、弱化、低对比度的背景。场景不得成为无关装饰，不得压过核心内容形成独立风景海报，也不得完全消失，使页面退化为白底框线、图标和箭头组成的普通信息图。
...
视觉优先级：页面核心判断 → 业务语境与能力形成 → 业务关系与价值结果 → 场景与环境支撑 → 必要的辅助元素。
