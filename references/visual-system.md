...
- **线条**：主关系用细、实、方向一致的深蓝线；次级边界或反馈可用浅灰短虚线，但整页反馈虚线原则上最多一条，虚线不作装饰节点链、不沿页面三边绕行，也不同时承担边框、回流和装饰。线端必须落在真实对象外边界；圆点只用于真实接口、汇聚或分支，禁止线端悬空、靠近但不接触、穿过文字、数字或关键对象。
- **边框**：整页可见边界最多两级：一级业务范围边界、一级必要子组边界。只用细线普通矩形或开放平面色场，边框只表达范围、归属、控制边界或独立对象；组内项目优先用留白、对齐、浅色底或短分隔线，不为每段文字重复套框。禁止胶囊、厚重框、梯形、切角、异形、玻璃舱和立体门框；允许一侧开放或局部边界。
- **箭头**：先用邻接、对齐、包含、留白和颜色表达关系，只有真实方向无法由空间关系读出时才使用箭头。编号、自然邻接或同一连续基线已经表达顺序时，不在每两个模块之间重复加箭头；整条流程优先只保留一条主方向线或一个末端箭头。箭头使用细实线和贴近线端的小型三角箭头头，箭头头宽度不明显超过线宽的2—3倍；禁止宽箭头带；禁止渐变块箭头、发光箭头、带状箭头、带阴影立体箭头、重复步骤箭头、中心辐射和跨文字连线。主业务流用深蓝细实线，反馈或复盘最多一条浅灰短虚线，分组关系用括号、对齐或开放边界而不用箭头；闭环用偏向一侧的开放路径或短回接线，不画完整圆环。
- **形状**：以平面直角矩形、开放色场和低矮、哑光、正视的微立体承载面为主；同页异形标题条最多一个。只有“平台承载、分层支撑、汇聚中枢”等业务语义明确时才允许低矮平台，且平台不得同时充当页面外框、标题底座和装饰舞台。禁止把普通栏目做成箭头带、徽章、盾牌、圆盘、中心辐射、梯形、六边形、切角容器、厚底座或多层台阶；禁止夸张挤出、复杂光效和展台化造型。锁定内容未明确要求时，不生成对勾、警告三角、循环图标、定位针、盾牌或装饰性连续箭头。
- **颜色角色**：深蓝表达主关系和结论，浅蓝灰承载辅助信息；暖色只标记风险、异常、限制、禁止或待处理状态。不得仅为区分类目自动分配多色或彩虹色，也不做彩色等级卡片墙。组件数量和重复次数服从业务关系，不为填满画布而增加线、框、箭头或形状。
