...
生成克制、清晰、高级的政企领导汇报页。优先使用可识别的业务场景、具体对象、动作、证据和结果；抽象主题则使用开放、非对称的平面2D编辑式关系场。当平台、中枢、引擎、中心等本身就是页面明确表达的业务对象时，可以成为中心载体或主关系枢纽：优先画成承接输入、转化、协作和结果的扁平业务关系场或工作环境，使真实业务对象、输入输出、边界和结果在其中发生可读联系，而非画成独立产品设备、机器展台、圆盘装饰或夸张发光装置。

默认媒介是一处开放、连贯且具有行业辨识度的业务场或具体对象面；页面先读到行业环境中的业务动作、对象状态或受控结果，再读到与其就近结合的文字关系。主业务锚点优先由真实设施、设备、作业环境、人员动作、物流或信息流等可观察对象共同形成；页面明确要求的平台、中枢、引擎或中心可以组织这些对象和关系，但资料容器或通用办公物件不得替代实际业务过程。行业场景与文字共同构成主视觉；多个文字组可以分布在同一连续关系场中，文字组数量不推导出配图数量，也不要求各自拥有独立卡片、图标容器或抬高面。只有当页面语义确实没有可识别的行业环境、业务对象或动作时，才使用纯平面关系表达。
...
- 相邻语义区域优先共用同一行业主场或连续动作；只有不可替代的关键转折才增加一个明显从属的局部证据。配图通过近景与远景、清晰与弱化、大小与裁切形成节奏，而不是让每个语义区域都拥有独立场景；至少一处行业场景采用大胆裁切或延伸至画面边缘，形成高质量报告封面级冲击力。
...
- 优先让行业对象、动作和结果成为主视觉；图标默认数量为零，只有页面脚本明确指定的具体图标才可作为从属细节出现。屏幕、资料或抽象几何可作为局部说明，但不应替代业务本身。
- 平台、边界、汇聚或协同关系可采用低矮、正视、哑光的平面关系面、开放色场或克制微立体；以材料层次、裁切和留白建立质感，而非舞台化或强光效。
- 用真实行业质感、纸面编辑节奏和深浅蓝层次表达科技感；避免落入软件营销、展陈海报或未来概念展示的视觉语气。
- 一个关系只做一次最有解释力的呈现；若需要收束，让它自然落在主线的结果、接口或边界上，而非另起一套流程或总结区。
- 文档、合同、报告、表单、屏幕或仪表盘可以在语义确有需要时作为局部证据；优先由行业设施、设备、环境、动作和状态承担画面主体。
- 文字与图像应在同一关系场中彼此解释：文字贴近对象、动作、接口、边界或结果，图像为文字提供可观察的语义依据。
...
把内容组织为一个连续且具有行业辨识度的业务场：真实设施、设备、作业环境、人员动作、物流或信息流共同承载核心判断，文字组贴近对应对象、动作、接口、边界或结果自然展开。正文较多时，把相邻内容做成同一现场的分层旁注、材料细节或沿路径的编辑批注；整页可以由同一行业环境中的多个关联对象共同成景，资料、屏幕和文件在需要时作为局部证据进入画面。
#### 分类或矩阵

semantic_tags: [classification, matrix]

把分类画成同一业务场中可辨认的不同对象、材料、动作或状态：它们可通过近景与远景、实物与局部细节、深浅与裁切形成节奏。分类名称像编辑注释一样贴近相应对象；相邻分类可共享同一环境、工作台、流向或结果，使观者先看到业务画面，再读到分类含义。局部平面结构用于梳理关系，但保持开放、轻盈并从属于主业务工作面。

#### 步骤、流程或输入输出

semantic_tags: [flow, input_output, sequence]

让流程或环节依附于同一业务对象或连续路径的状态变化：观者可以沿材料、设备、作业、接口、运输、交付或使用中的真实变化阅读先后。不同输入、输出或节点用位置、状态、色调、材质和与主对象的距离区分；连接线只轻量提示流向，让动作本身成为阅读的主导。

#### 权利边界

semantic_tags: [boundary, authorization]

把权利、责任、准入、控制或使用范围表达为与业务对象直接相关的边界、门控、内外位置或受控状态：例如接口前后的可见差异、进入工作区的权限、责任双方的交接点或受控材料的流向。让边界成为业务画面的可观察部分，并由附近文字完成说明。
...
保持扁平2D、克制政企气质与高端咨询报告的编辑感。先把页面画成一幅可阅读的业务画面：以一个占主导的行业场景、主关系或平台载体建立视觉命题，再让关键判断、转折和证据沿同一阅读主线展开。平台、中枢、引擎等明确业务对象可成为真正的关系枢纽，并以输入、输出、边界和结果使其含义可见。有明确行业语义时，可配置多处与主关系直接相关的场景、对象切片或动作画面，但相邻文字组优先共享同一连续业务场、主关系或对象动作；不得按文字条目机械地一文一图。只有某个关键转折或证据确实无法由主场和相邻关系解释时，才使用明显从属的局部画面。每处配图都应明确解释对应的业务对象、动作、状态、边界或结果，并与文字就近附着、穿插或共同形成关系。用大胆裁切、大小差、前后景、清晰度和跨区流向形成主次；至少一处行业场景延伸至画面边缘。文字像编辑注释一样贴近对象、动作、接口、边界和结果，深蓝用于关键判断与主关系，纯白和低纹理空间让阅读呼吸。可使用少量局部承载面、细线和资料性证据，但它们从属于业务场与文字关系。图标、徽章和抽象符号默认不出现；只有【内容锁定】明确要求的具体图标才可作为小型从属细节，绝不用于替代分类、流程、角色或文字。若提供风格参考图，只继承色彩、材质、留白、字体层级和编辑节奏，不复制固定物件或分区。让一个必要收束自然落在主线结果上；页面语义、锁定文字和唯一视觉命题优先于装饰与固定版式。
配图粒度必须跟随主关系而非文字条目：整页优先由一个连续主场、同一对象或连续动作承载多个文字组及其关系；只有必要时增加一个从属的转折或结果单元，而非把每个文字组变成完整图像单元。独立局部画面不应与主场等权，也不得为每个主要文字组各设完整画框、照片切片或承载面。明细文字应像围绕主对象的连续注释带、就近标注或同一作业链上的文字节点出现，不在每一项下方或旁侧配一张缩略图、屏幕、物件照片或独立场景。多个文字组可以围绕同一对象、同一动作链或同一空间连续分布，并通过文字层级、邻近和少量引线区分，不必逐项配图。不得用只证明行业身份的全景照片替代对具体业务对象、动作、状态和结果的表达。

保持页面整洁：从主线索进入，在关键判断或结果处收束；主视觉之外的信息以安静的就近标注、局部细节或低对比背景存在。连接沿阅读方向轻量推进，行业场景只在能解释业务语义时进入构图，并通过裁切、透明度或景深服务主线。

保持整洁但不牺牲图文融合：减法首先作用于重复标题条、重复容器、无关背景和装饰性连线；保留真正解释文字的语义配图。让同一主场中的多个文字组与共享图像合并为一个主视觉或从属关系组，并以不等大小、裁切、重叠和留白形成自然节奏，而非机械等宽排列。

当页面包含分类、服务形态或多维明细时，优先采用由同一真实业务环境贯穿的编辑式对象关系：选取一个主对象或连续作业动作，再以至多一个互补局部细节说明不可替代的转折，让文字以旁注、引线或贴近对象的短块进入画面。不要从每个分类分别抽取一张完整切片；画面像一张有主次的咨询报告跨页，而不是软件产品页。先寻找可以被看见的业务对象，再用图像与文字共同说明分类。除非页面脚本明确指定具体图标，图标和抽象符号不进入画面；界面只作必要的辅助语言。

选择画面媒介时，先选可观察的真实对象、动作或成果状态，再选与之相连的编辑式文字标注，最后才使用图表或界面；抽象符号默认不使用。密集分类可以借助字号、字重、留白、引线和对象邻近关系形成层级：让一组相邻文字共享同一对象切片或工作场，而不是为每个名称另设相同的图形容器。由文字驱动的关系图也应像一篇被图像穿插的编辑版面，具有呼吸、节奏和重点，而非整齐的组件目录。

【图标硬约束】除非【内容锁定】明确指定某个具体图标，否则不得生成任何图标、象形符号、徽章、勾选标记、人物剪影、数据库/书本/大脑/盾牌等替代性符号；分类、流程、角色和文字必须由真实对象、动作、材料、场景切片与编辑式文字关系表达。

【观看焦点】先设计观者的前三秒：首眼只落在一个主载体或主场景，它以最大尺度、最清晰边缘、最高文字对比或最强空间张力占据约55%—65%的视觉注意力；第二眼才落在一个关键转折、接口或结果，占约20%—25%；其余文字组作为低对比、较小尺度、局部裁切或背景化的证据沿主线附着。所有内容仍须完整可读，但不得让每个文字组、照片切片或承载面都拥有同样完整的画框、对比、面积和细节。主载体可以是平台、中枢、连续行业动作或结果，而非默认的中央圆盘；画面应先有一个可记住的镜头，再有完整的信息。
