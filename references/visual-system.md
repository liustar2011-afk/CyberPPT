...
优先采用场景化行业插画、编辑式业务场景或具有行业语义的综合视觉表达。场景形式和视觉重量由页面内容决定，可作为局部主视觉、结构内嵌内容、中景环境，也允许作为虚化、弱化、低对比度的背景。场景不得成为无关装饰，不得压过核心内容形成独立风景海报，也不得完全消失，使页面退化为白底框线、图标和箭头组成的普通信息图。
