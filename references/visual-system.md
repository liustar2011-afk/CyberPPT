...
面向政企领导汇报与演讲辅助，采用象牙白 #F7F6F0 与深蓝 #12355B。整体专业、克制、大气，视觉身份为 senior leadership briefing / speech-support，不采用咨询交付件、学术论文、软件产品宣传或传统流程信息图的默认审美。
先识别页面核心判断、业务对象、证据类型及其关系，再决定位置、尺度、分组、阅读路径和视觉载体。内容决定构图，不从预设版式、固定模板、后端构图字段或习惯性布局骨架出发。页面形成一条清晰阅读路径和一个主叙事中心，但不强制居中主物体，不做机械均分。

风格只决定页面的视觉气质，不预设页面必须使用场景、插画、照片、图标、三维物件或任何固定媒介。视觉载体必须由页面语义另行决定：数据治理、制度机制、标准体系、权限关系、知识结构、系统能力等抽象主题，优先采用二维编辑结构、关系场、数据纹理、抽象材料层和精细排版；只有页面明确依赖人物、设备、地点、环境或操作行为才能成立时，才使用场景化表达。

优先建立高级编辑式业务语境，而非展示行业景观或把抽象概念具象化为陈旧办公物件。通过非对称平衡、尺度对比、精细排版、发丝线、克制的数据纹理和充足留白表达业务能力如何形成、协同、运行、支撑决策并产生价值。不得默认使用档案柜、文件夹、牛皮纸、教材照片、数据库圆柱、服务器机柜、控制台或通用设备来代表知识、数据、平台、治理、权限和生命周期。

场景、照片或编辑式行业插画是条件性载体，不是风格9的默认组成。仅当场景能够直接解释本页不可替代的业务动作或物理环境时使用，且通常作为局部、低对比、从属于主关系的辅助层；当抽象结构比场景更准确时，应完全省略场景。禁止为了“丰富画面”强行添加行业设施、人物、环境或逐模块配图。

页面关系首先通过版面层级、位置、尺度、间距、对齐、密度与留白表达，不默认绘制节点、链路、箭头、光束或技术面板。只有关系无法通过编辑排版清楚表达时，才允许使用少量发丝线或低对比辅助标记。文字与视觉构成应组成同一叙事，不得简单处理成“背景图片＋前景信息图”，也不得把抽象业务关系自动翻译成软件架构图或流程图。

对于数据治理、制度机制、标准体系、权限关系、知识结构、系统能力等抽象页面，默认采用“编辑排版型”媒介：以精细中文排版为主体，以一处克制的深蓝形面、局部数据纹理或抽象材料层建立视觉重心；业务关系只影响文字块的尺度、位置和分组，不绘制完整流程、节点序列、架构层、技术光束或逐项连接线。

除非页面内容本身明确要求，禁止等权卡片墙、编号步骤卡、时间线、中心模块加卫星节点、机械流程模板、四栏咨询排版、SaaS界面、仪表盘布局、软件架构图观感和技术规格图观感。页面中的信息应有明确主次和视觉权重差异，主叙事统领全页，避免多个彼此割裂的视觉中心和视觉语言。

避免把业务含义处理成圆形图标、图标墙、线性符号、扁平界面组件或等距三维小组件。也避免复古档案美学、厚重拟物容器、设备陈列和用物件字面替代抽象关系。
...
视觉优先级：页面核心判断 → 业务关系与能力形成 → 价值结果 → 与语义匹配的视觉载体 → 必要的辅助元素。风格负责气质，页面语义负责媒介，二者不得混为同一层规则。
