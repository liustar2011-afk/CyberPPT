...
默认媒介是一处开放、连贯的业务场或具体对象面；页面先读到一个业务锚点，再读到文字关系。主业务锚点优先表现可观察的业务动作、状态变化或受控结果，不让抽象概念代表物替代实际业务过程。锚点在内容允许时占约35%—50%的视觉权重，2—5个文字组共享同一视觉场，不要求独立卡片、图标容器或抬高面。只有当页面语义无法由场景或对象承载时，才使用纯平面关系表达。

生成优先级：核心判断 → 业务动作或状态 → 主业务锚点 → 文字关系 → 辅助证据 → 必要连接。后一级不得压过前一级，装饰原则上不主动增加。
...
- 中文字体统一使用微软雅黑（Microsoft YaHei）；所有可读文字的视觉字号不得小于 14pt 等效尺寸。空间不足时应精简视觉元素、扩大文字区或调整构图，不得通过缩小字体解决。
...
- **边框**：保留既有容器形状和数量，只优化描边质量。边框使用细、低对比、单层描边；同一层级的线宽、颜色、透明度及圆角或直角处理保持一致。边缘清晰但不抢文字，不使用双描边、高亮轮廓、发光、厚阴影、内外多重描边或立体门框；局部开放边界的断口应整齐，并与内容对齐。
...
- **形状**：保留已经选择的载体类型、轮廓和数量，只优化表面与边缘。同层形状的直角或圆角、描边、明度和厚度保持一致；平面色块边缘清楚，微立体承载面保持低矮、正视、哑光，仅用极浅的前后差。不得把结果区、承载面或平台加厚成展台、多层底座或台阶，不增加高光金属边、玻璃质感、夸张倒角、挤出、悬浮和复杂光效。
...
把内容组织为一个连续的业务工作面，选择一个能承载核心判断的业务锚点（如受控服务场、合同或成果对象、运营工作面、证据场），让文字组围绕、附着或穿过同一视觉面展开。正文很多时优先扩大主视觉面和文字区，不通过压缩文字、增加小卡片或堆叠标签解决密度。整页最多一个主场景或主对象、一个辅助证据对象；禁止一类一图、缩略图拼贴、照片条带和多张小场景并列。
...
保持扁平2D、无图标优先和克制政企气质。以一个可观察的业务动作、状态变化、受控结果或具体业务对象作为主锚点，文字在同一业务场形成不等权关系。连接只表达真实关系并保持细、小、从属，不支配页面。页面语义和锁定文字优先于装饰与固定版式。
