...
优先构建业务结构，而非铺陈行业景观。对于治理、权限、标准、知识结构、系统能力等抽象主题，重点表达业务对象、证据、关系、边界和结果，而非罗列技术名词；优先使用精细中文排版、版面分组、对象叠置、局部放大、尺度对比和留白表达关系。半立体对象只能作为局部语义载体，不得形成圆台、圆盘、卡片堆、模块墙、容器组或设备陈列；禁止用档案柜、文件夹、牛皮纸、数据库圆柱或服务器机柜字面代替抽象概念。
...
图形应从文本逻辑派生，将输入、输出、依赖、反馈、演进、协同、对比和价值关系转化为空间组织、对象联系和状态变化。优先使用文字尺度、版面分组、对象叠置、局部放大和留白表达关系；线条、箭头、容器和图标不是默认视觉语言，只有页面语义明确要求时才允许使用。不得机械地把每项文字转换成一个图标、卡片或独立模块。文字、图形与业务对象共同组成一体，不得简单处理成“背景图片＋前景信息图”。
