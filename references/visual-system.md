...
- **送图脚本门禁**：调用 ImageGen 前，必须把将送入生图工具的明文 prompt 落盘到 `workbench/prompts/imagegen/`，在对话中展示，并等待用户修改或批准。送图内容只含主判断、上屏文字与视觉结构；只有页面主题本身属于范围、准入、安全、治理、质量、合规、风险、保障、部署、容量、降级或验收时，才允许编入与主题直接相关的限定内容。普通主题页不得把内部边界带入送图 prompt；禁止夹带完整文字稿、取舍说明、证据映射、证据编号、讲解提示。
