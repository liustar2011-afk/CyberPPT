...
Style 09 是纯白底上的深蓝编辑式领导汇报：画面先让人感知到与主题有关的业务对象、动作、环境或结果，再自然读到文字及其关系。它服务于页面的既定判断和版式，不替代 Stage02 决定业务关系、阅读顺序、对象数量或空间布局。

### 正向构图语言

- 以纯白留白、深蓝主关系、浅蓝灰辅助面和克制细线建立清晰、安静而有张力的视觉秩序；文字、对象、场景和关系在同一画面中共同表达。
- 形成清楚的视觉焦点与阅读主线：允许平台、中枢、引擎、中心等页面明确的业务对象成为主载体，也允许连续行业场景、关键动作、结果状态或开放关系场成为主载体；具体选择由页面语义和 Stage02 结构决定。
- 用尺度、裁切、前后层次、明暗、对齐、留白和局部色场形成主次；需要多处配图时，让每一处直接解释相应的业务对象、动作、状态、边界或结果，并与文字就近结合。
- 锁定文字保持完整、清晰、舒展；文字像编辑注释或业务信息一样贴近对象、接口、动作、边界和结果，而非落入独立的文字孤岛。
- 行业场景、设备、工作面、信息流、资料、屏幕、设施和人物可按页面语义作为视觉载体；它们必须解释内容，不作为泛科技装饰。抽象主题可使用干净的平面关系场。

### 表面与组件语言

- 信息结构以平面2D、正视、哑光和编辑式留白为主；真实场景可保留自然透视和物理体积。避免展陈、SaaS 界面、未来概念海报式的夸张光效或材质。
...
- 页面表达应整洁但不均质：允许多图、多对象和多处局部细节，只要它们共同形成可读的主次、节奏和业务解释，而不是彼此孤立的装饰区。
...
保持纯白底、深蓝主关系、平面2D信息结构和无图标优先。先遵从页面既定的主判断、业务关系和阅读顺序，再用一个清楚的视觉焦点组织场景、对象、文字与证据；主次通过尺度、裁切、留白、对比和前后层次自然建立。锁定中文必须完整清晰，贴近对应对象、动作、接口、边界或结果；多处配图和局部对象可以共存，但每一处都应解释相应文字的业务意义并共同构成同一画面。行业场景、资料、设备、屏幕或人物只在确实说明业务时使用，不作为泛科技装饰。连接线和承载面保持克制、清楚、服务关系。图标默认数量为0，不得以图标、徽章或抽象符号替代业务对象、关系或文字。最终效果应像高级领导汇报中的编辑式业务页面：整洁、有焦点、有图文融合，也保留足够的构图自由度。
