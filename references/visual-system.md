...
面向政企领导汇报与演讲辅助，采用象牙白 #F7F6F0 与深蓝 #12355B。整体专业、克制、大气，围绕页面核心判断形成一条清晰的业务叙事主线，重点表达能力、状态、价值或结果。
优先采用场景化行业插画、编辑式业务场景或具有行业语义的综合视觉载体，不要求照片式实景。先根据页面判断、业务对象及其关系选择主视觉表达，不预设背景图、流程图或固定版式。
行业场景可以作为局部主视觉、中景环境、结构内嵌内容，也可以作为虚化、弱化、低对比度的背景。其位置和视觉重量由页面内容决定。场景应参与表达业务能力和逻辑关系，不得只是装饰；避免完整风景、城市天际线或能源设施群像成为独立主画面，也避免在大幅背景图上悬浮粘贴一套无关的流程结构。
图形应从文本逻辑派生，将因果、输入输出、依赖、反馈、演进和价值关系转化为空间组织、对象联系、状态变化和作用路径。允许使用必要的标签、线条、图标和模块，但不得主要依靠一模块一图标、等权卡片、编号步骤、中心辐射、机械流程、软件架构或仪表盘完成整页表达。
场景化表达应与核心业务结构形成一个统一视觉中心。避免两个极端：一是行业场景压过内容，形成风景海报；二是场景完全消失，退化为普通白底信息图。
人物默认不出现。仅当页面语义确实需要人的操作、判断或协同时，才允许少量、远景、低对比度人物作为辅助；禁止正面人物、近景人物、可识别面部、会议摆拍和企业宣传摄影。
