...
Build one integrated, asymmetric and unequally weighted composition around the selected semantic focus.

Every page must preserve the following visual DNA:

- pure-white background with restrained deep-blue hierarchy;
- one source-grounded, visibly dominant semantic focus;
...
Icons must not determine the composition; they may only clarify content relationships already established by the source-grounded focus.
...
Before choosing a composition, select one page-specific visible semantic focus from explicit source content. The focus may be a fact, judgment, metric, business object, relationship, action, stage or state. It must give the supporting content a clear reason to coexist on the page.

When the page title, page mission or core judgment is marked as non-visible metadata, do not render or paraphrase that metadata. Select an explicit source fact, object, relationship or organizing axis as the visible focus instead.

When the source contains parallel propositions without one dominant claim, use their explicit common object, dimension or relationship as the focus. Preserve the parallel structure and do not invent a synthetic conclusion.
...
Repeated or equally weighted cards are allowed when they provide the clearest grouping for genuinely parallel information. Choose cards from semantic roles and reading needs, not from the mere presence of bullet points.
...
Only content that directly explains, proves, qualifies or extends the selected focus should receive visible space. Treat the remaining supplied copy as semantic context; do not attempt to place every available sentence, module or label on screen.
...
Preserve the complete meaning, factual boundaries and claim strength of the selected visible content. Achieve readability through evidence selection, hierarchy and grouping; do not crowd the page with every available detail.
...
Use an executive-report visual language in which photography, object illustration,
relationship graphics, data-shaped forms and editorial structure are equal candidate
carriers. Photography is optional and must not become the default carrier merely
because the page concerns an industry or contains abstract policy language.
...
Select visual carriers only after identifying the page's semantic focus and dominant
relationship. Industry category establishes context but does not by itself determine
whether a scene is needed or which carrier should dominate.

Choose the number, scale and distribution of visual carriers from the page meaning.
A page may use one scene, several independent scene fragments, multiple object crops,
illustrated business objects, relationship graphics, data-shaped forms, or a mixed
composition. Multiple visual fragments do not need to form one continuous panorama.
They may support nearby content individually, work together as a semantic family, or
provide a restrained environmental cue that helps establish context and reading rhythm.

Do not default to one large photograph with the remaining content arranged as a dense
text block or a row of text-heavy cards. Let visual material appear at more than one
scale or location when separate content groups have different drawable objects,
actions, states or evidence. A large dominant scene is appropriate only when the scene
itself carries the page's central evidence or relationship.

A dominant scene must visibly encode at least two of these semantic elements: actor,
action, business object, condition, state transition, relationship or outcome. A
dominant scene that only signals an industry category is insufficient.

Medium and small scenes, image fragments and object crops may each carry one specific
object, action, state, place, process detail or contextual cue. They may be partial,
atmospheric or documentary, and they do not each need to explain the complete page
logic. Judge their semantic contribution collectively and in relation to the nearby
content. Decorative use is acceptable in restrained amounts when it strengthens visual
rhythm, depth or atmosphere without introducing a conflicting meaning.

Apply a replaceability test before finalizing the scene: if the same picture could be moved unchanged to many unrelated pages in the same industry, make the depicted action, object, transition or relationship more page-specific.

Do not let substations, transmission towers, renewable-energy bases, control rooms,
dashboards or other familiar industry imagery dominate a page solely as generic
decoration. They may appear as medium or small contextual material even when their role
is broad, provided the overall composition remains relevant and does not imply a false
fact or relationship.

Cards may organize supporting copy, evidence and visual material. Photography inside a
card may carry a direct semantic role, a contextual cue or a restrained atmospheric
function. Image fragments may also sit outside cards, interrupt a structural band,
anchor a local explanation, or bridge adjacent content groups.

Integrate visual material directly into information regions through:
- soft edge fading;
...
Do not place every visual fragment inside a rigid card.

Visual material should feel distributed through and fused with the information
structure rather than concentrated in one photographic block or pasted onto it.

### Optional scene mapping guidance

The following are examples when a real-world scene is semantically useful. They are not
default scene assignments and do not override the carrier-choice rules above.
...
Every page must establish one strong, source-grounded semantic and visual anchor.
...
Selected supporting information must remain subordinate in scale and contrast while staying readable. Unselected details remain semantic context and do not require visible placement.
...
Evidence areas may use equal or unequal cards according to whether their source-supported roles are parallel or hierarchical.
...
Symmetrical side-by-side cards are appropriate for genuinely parallel evidence; use unequal weight when the source establishes an asymmetry.
...
The bottom band may use a continuous structural carrier or repeated cards according to the semantic relationship of its content.
...
Every page must establish one visually dominant focus grounded in explicit source content, making the page's organizing meaning immediately recognizable.

The focus may be an explicit fact, conclusion, key metric, business object, scene, critical relationship, mechanism, platform, stage, shared organizing axis or risk state. Do not invent a conclusion when the source only provides parallel propositions.
...
Selected supporting information must remain subordinate while readable and must directly explain, prove, qualify or extend the focus.
...
Do not place all supplied copy merely to demonstrate completeness. Preserve the meaning and boundaries of the selected visible evidence, and keep non-selected details as semantic context.
