...
> Per-page visual self-check rubric for slide SVGs. Read by the subagents spawned during the `visual-review` stage. Companion to the [`visual-review` stage](references/visual-review.md) and the [`visual_review.py`](../scripts/visual_review.py) renderer.
