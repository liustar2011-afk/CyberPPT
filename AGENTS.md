...
- Stage 01 内容页的上屏文字必须形成可独立阅读的层级结构：每页应先明确分组标题，再承载组内短语、事实和数值；不得把不同类别内容平铺为同一层级清单。分组标题、组内归属、信息密度和阅读主线属于 Stage 01 内容职责，未形成清晰分组时不得进入 Stage 02 生图。
- Stage 01 上屏分组不得把 `1、2、3、4`、`①②③④` 等数字编号作为通用分类标记。只有来源和页面主论证链明确存在流程、阶段、时间、优先级、门控或其他真实顺序时才可使用编号；并列分类、职责、能力、对象、边界和条件使用无编号的业务分组标题。
- Stage 01 短语化不得把一个共享标题、谓词、对象、限定语或结果重复复制到多个同级条目中。共享语义由父级分组或一条总括语承载一次，各子项只表达自身新增信息；若两个条目删除不同前缀后仍保留相同明细，必须合并或重建为“共享父级—差异子项”结构。
