...
- 上屏并列项必须共享一个明确维度，如主体、能力、阶段、问题、任务或结果。不得把主体、业务领域及该主体的评价结果放在同一层级。补充事实、成熟度评价、认证或结果先归入其修饰对象；faithful 模式按 `full_copy` 的来源层级选择上屏内容，analytical 模式可围绕已批准的核心结论组织证据层。
...
- 完整语义必须明确具体事项。子项只能继承同一可见模块直接声明的共同主语或动作，且标签必须明确语义角色；`full_copy` 和 `onscreen` 不得依赖上一段、相邻模块、上一页或读者猜测来补全关键业务对象。faithful 模式允许来源正式定义的分类、阶段、主体、任务名称直接作为模块标题；其子项不得添加来源未说明的作用或结果。analytical 模式的普通模块标题按获批分析结构表达完整判断。确定性长度门禁属于生产约束，不是写作目标；超限时通过拆分语义角色、调整页面使命或分页处理，不得截断关键对象、动作、关系、条件或限定。AUTHOR 逐页执行“锁定来源范围与保护信息—精确原文回读—完整稿语义保全—上屏投影—来源力度与边界复核—整页重写”的内部闭环；该闭环不新增权威状态文件。
