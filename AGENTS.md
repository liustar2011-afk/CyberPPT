...
- `full_copy` 是 Stage 01 内容页的唯一正文稿。AUTHOR 必须回读页面 `source_refs` 对应原文，保留核心事实、正式主体、文件名称、建设状态、任务强度、数字节点、责任、条件、边界以及来源明确作出的结论；不得用“建设内容、阶段进度、技术规则”等作者归纳维度替换原文中力度更强的政策动作、实施状态、明确目标和成果定位。多段文字继续采用观点先行和完整语义表达。Stage 01 直接将该页 `full_copy` 原样复制到 `onscreen`，不再单独压缩、拆分或设计视觉结构；v2 lean 的 `source_refs` 仍表示本页可用证据范围。
- “短语化、条目化”不再作为 Stage 01 的上屏生产目标。上屏内容沿用 `full_copy` 的原有段落、事实、业务对象、动作、关系和必要限定；Stage 02 负责文字可读性、图像文字审计和最终页面 QA。
