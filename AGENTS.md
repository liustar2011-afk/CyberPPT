...
- `full_copy` 必须围绕 `core_message` 对本页来源内容进行结构化、层次化重组，其主要任务是保存和组织来源语义，不承担概括压缩。AUTHOR 必须回读页面 `source_refs` 对应原文，保留核心事实、正式主体、文件名称、建设状态、任务强度、数字节点、责任、条件、边界以及来源明确作出的结论；不得用“建设内容、阶段进度、技术规则”等作者归纳维度替换原文中力度更强的政策动作、实施状态、明确目标和成果定位。多段文字默认采用观点先行：优先使用来源已经明确作出的最强结论作为段首观点，再以“一是、二是、三是”等符合真实关系的结构展开完整依据；当标签能够更清楚地暴露证据、范围、动作、条件或结果时，也可使用“语义标签：语义完整的短语或说明”。只有来源没有明确结论时，才允许在证据边界内形成受约束的归纳。AUTHOR 可以改写、调整顺序、合并重复内容，并省略与页面核心结论无关的从属材料；不得省略支撑页面结论成立的核心信息。内容压缩和显示取舍主要发生在 `onscreen`：上屏只保留核心结论及决定性依据，不得对完整文字稿做等比例缩写。v2 lean 的 `source_refs` 表示本页可用证据范围，不要求无差别照录每条来源文字，也不为省略的非核心细节新增清单或审批文件。
