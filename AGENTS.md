...
- **新项目统一作者化到 Final Script 1.2。** 内容页由 Stage 01 产出 `full_copy + fidelity_text + source_refs + source_provenance`；1.2 内容页不得 authored `onscreen`。Final Script 1.0/1.1 仅保留旧项目读取与原位兼容，不得作为新项目默认写作合同。
- `full_copy` 是 Stage 01 内容页唯一完整正文语义稿。AUTHOR 必须回读页面 `source_refs` 对应原文，保留核心事实、正式主体、文件名称、建设状态、任务强度、数字节点、责任、条件、边界以及来源明确作出的关系或结论；不得用作者归纳维度替换原文中力度更强或更具体的业务动作、实施状态、明确目标和成果定位。faithful 模式按来源原生结构直接形成完整稿，来源没有单一中心判断时保留并列命题；analytical 模式才可在来源边界内采用观点先行和获批分析结构。
- `fidelity_text` 只保存极少量必须保持字面准确的原子字符串，并显式标注 `required` 或 `if_rendered`。可进入的典型内容包括精确数字及必要单位、政策/规划/标准/机构正式全称、用户明确指定的专有名词和固定字符串。普通结论、整句、整段、可安全改写的业务措辞和风格性表达不得进入 `fidelity_text`；不得用保真项重新制造一个隐藏的上屏稿。
- `full_copy`、可选核心判断/论证字段、演讲者备注和 `fidelity_text` 必须继承原稿的发布主体与陈述立场。原稿直接定义、要求、安排或陈述的内容，脚本也直接陈述；严禁增加“通知所称”“材料指出”“文件认为”“原文说明”“在通知中已明确”等第三方转述框架。原稿本身对上位政策、制度或其他文件的引用，应按原关系保留。
- `faithful` 模式下页面标题默认保留来源原标题；同一来源章节拆成多页时采用“原标题＋页内范围限定”区分页面。仅在来源没有可用标题、用户明确要求分析型标题或批准结论式改写时重新拟题。
- Stage 01 不为视觉密度预压缩完整稿，不设置上屏字数或模块数目标。页面语义范围过大时优先调整页面使命或分页，不得以删减事实换取篇幅。关键限定、业务对象、动作、关系、责任、时间和数字必须在 `full_copy` 中保留。
- Stage 02 对 Final Script 1.2 执行统一内容适配：内部稿由 `full_copy` 派生运行时 `onscreen_text`，外部稿由结构化 `内容` 字段或自由正文派生；普通内容允许选择、改写、合并、精简、拆分和重排，`fidelity_text` 始终独立传递并且只有保真项接受精确字符串验收。不得把普通 `full_copy`/运行时 `onscreen_text` 重新升级为全文逐字权威。
- 同一事实跨页复用时，每一页必须承担不同且明确的内容角色；仅重复列举的事实应删除、降级或改写为当前页真正需要的来源内容。全稿 Critic 必须检查相邻页重复和遗漏，不得为了制造“结论页”而新增来源未表达的综合意义、责任或价值判断。
- 自读页面中的体系代码、编号和缩写在承担核心关系时必须与业务名称和作用同页可获得；声称与国家节点、项目节奏或阶段目标衔接的页面，其 `full_copy` 必须同时保留时间或触发条件及各阶段新增状态。Stage 02 在生成可见内容时不得改变这些保护语义。
- 页面表达映射、协同、闭环、衔接、转化或支撑关系时，只有来源或获批 analytical 推理明确支持该关系，才能进入 Stage 01 语义稿和关系字段；Stage 02 只能表达已批准关系，不得新造业务关系。
- 演讲者备注在存在时必须提供完整语义稿之外的增量价值，至少承担依据解释、次级证据、不改变结论力度与适用范围的次级条件边界、听众关注点或自然过渡之一；改变结论力度、时间、责任或适用范围的关键条件必须进入 `full_copy`，不能只放在备注中。faithful 模式允许省略演讲者备注。
- 所有新项目的 Deck Plan 均使用 v2 lean；profile 只决定 Foundation 的理解深度和来源保全方式，不决定页面规划合同。strict/legacy 继续保留 Source Truth、完整语义模型、逐事实核验和 `source_consumption_policy: required`，Deck Plan 只承担章节归并、页数分配、暂定标题、页面问题/使命和来源边界；核心判断、完整正文、保真项、视觉关系与讲述线索属于 AUTHOR 和 Final Script。v1 strict Deck Plan 仅供已有旧项目原位兼容，不得作为新项目默认模板。Stage 02 不读取 Deck Plan 文案。
...
来源章节与汇报章节必须分层。Foundation 保留来源章节身份、边界和顺序；Deck Plan 默认把相邻来源章节按共同受众问题、内容角色和承接关系归并为汇报章节，映射展开后必须覆盖全部来源章节且顺序一致。正式汇报优先控制在 4 个汇报章节以内，默认不得超过 6 个；超过 6 个必须记录具体例外理由。多章节汇报中每个汇报章节设置一页过渡页，页面序列为封面、目录、逐章过渡页与内容页、封底。单章节规则继续适用。

首次 PLAN 前明确用户用途。Deck Plan v2 必填 `delivery_mode`（演讲辅助 `presented` / 独立阅读 `self_read`）及 `pagination_rationale`（结合用途与已知时长、页数约束说明实际分页理由）。两字段属于规划元数据，不预制上屏文案或字数配额；主 Agent 据此调整单页范围，保留完整语义。Final Script 的 `deck.delivery_mode` 与规划一致，Stage 02 沿用已选用途。用途变更须复核分页并更新失效预检；已有 v2 规划缺字段时依用户意图补齐，不自动默认用途。

两种用途均遵守“一页只表达一项核心内容”。同一核心内容可有多条支撑事实、并列事项或完整流程，独立主题分别分页；复杂主题可按子问题拆分。用途只调节该核心内容的解释展开与讲解负担，独立阅读仅合并服务于同一核心内容的信息，不要求来源没有的总判断。
...
- 不得根据审计错误机械增加页面、正文段落、保真项、锚点句或附件字段。来源覆盖只证明可追溯，不证明内容与页面使命直接相关。
