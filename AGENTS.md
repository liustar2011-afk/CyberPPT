...
- `full_copy` 必须围绕 `core_message` 对本页来源内容进行结构化、层次化重组，其主要任务是保存和组织来源语义，不承担概括压缩。AUTHOR 必须回读页面 `source_refs` 对应原文，保留核心事实、正式主体、文件名称、建设状态、任务强度、数字节点、责任、条件、边界以及来源明确作出的结论；不得用“建设内容、阶段进度、技术规则”等作者归纳维度替换原文中力度更强的政策动作、实施状态、明确目标和成果定位。多段文字默认采用观点先行：优先使用来源已经明确作出的最强结论作为段首观点，再以“一是、二是、三是”等符合真实关系的结构展开完整依据。只有来源没有明确结论时，才允许在证据边界内形成受约束的归纳。AUTHOR 可以改写、调整顺序、合并重复内容，并省略与页面核心结论无关的从属材料；不得省略支撑页面结论成立的核心信息。内容压缩和显示取舍主要发生在 `onscreen`：上屏只保留核心结论及决定性依据，不得对完整文字稿做等比例缩写。v2 lean 的 `source_refs` 表示本页可用证据范围，不要求无差别照录每条来源文字，也不为省略的非核心细节新增清单或审批文件。
- “短语化、条目化”只描述上屏明细的表面形式：可以删减虚词和句末标点，必须保留业务对象、动作、关系和必要限定。该要求不适用于把 `full_copy` 段落观点或 `onscreen.heading` 缩成孤立名词标签。来源正式定义的分类名可以保留，但同一可见模块必须同时说明该分类规范什么、解决什么或承担什么作用。
- 完整语义必须明确具体事项。`full_copy` 的段落观点和 `onscreen.heading` 不得依赖页标题、上一段或读者猜测来补全业务对象；“国家已明确……”“项目将推进……”“研究形成成果”“后续开展工作”等表述必须写明具体部署、项目、研究成果或工作事项。
