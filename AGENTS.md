...
- 轻量 Stage 01 采用“作者任务在前、规则质检在后”的正式工作方式。语义理解负责还原来源论证并完成原子事项与重要级别判断；Source Truth 是该作者判断的确定性事实投影；`compile-outline-draft` 只生成完整候选清单，不得直接作为正式提纲提交或通过人工门；必须调用仓库内 `cyberppt-author-stage01-outline` 完成页面使命、不可替代价值、主论证链、证据职责和不上屏取舍后，才运行一次 Outline 轻量审计。页面脚本使用 `cyberppt-write-single-page` 逐页专业编写，审计只作为来源、关系、层级和契约底线，不替代编辑判断。
