    ...
    *,
    ...
    if prompt_compiler == ARTIFACT_PROMPT_COMPILER:
        ...
        compilation_rules = [
            ...
            "- 页面任务、核心判断与主导关系用于理解业务关系；模型可自由组织上屏文字表达，完整上屏内容作为参考进入 full 图。",
        ]
        if visual_structure_mode == "review":
            compilation_rules.extend(
                [
                    "- 已显式启用视觉结构审阅模式：在内容参考之后加入主导关系、空间组织、阅读路径、载体和退化禁项。",
