...
AUXILIARY_LABEL_BUDGET = """【辅助标签预算｜不上屏】
本页辅助标签白名单为空。不得从正文自动提炼或概括任何新短标签；除【完整上屏内容】原有文字外，不得增加节点名、阶段名、输入输出词、状态词、底座名、图例、图标说明或侧边标签。
拉丁字母、英文缩写、单字母节点和仿界面令牌同样禁止；不得新增 Q/A、IN/OUT、INPUT/OUTPUT、YES/NO、状态码、按钮文字或伪界面字段。
关系必须依靠无文字的形态、方向、层级、邻近、包裹与视觉重量表达。未来只有上游脚本显式提供非空辅助标签白名单时，白名单内文字才可出现。"""
