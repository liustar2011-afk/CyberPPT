...
场景化插图中的人物、标牌、屏幕文字和数字仅作为环境质感，应采用远景、侧背面、浅景深、低对比或适度虚化处理，不能清晰地出现组织机构名称、人员名称和文件名称。中文字体统一采用微软雅黑或与微软雅黑字形特征接近的现代无衬线黑体。字号层级控制在4—5级以内，以正文为基准，页面主标题约为正文的1.6—1.8倍，一级模块标题约为正文的1.25—1.4倍，二级标题约为正文的1.1—1.2倍，注释约为正文的0.8—0.9倍。
