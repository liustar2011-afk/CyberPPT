...
这套跨页面展开的图形构图是主要组织层，不受插图矩形容器限制；其视觉表现力必须高于普通方框、通用连接线和原生办公软件框线图。
生成页面前先区分主体、支撑、输入、输出、分支、汇聚、闭环、层级、对比、因果与结论等关系角色，再决定视觉主线；不得仅按文本顺序机械排列或连接。
每个锁定模块及其名称只能承担一个主要关系角色并出现一次；模块一旦成为中心、底座、主轴、入口、结果或正文模块，不得再作为外围节点、关系标签、重复卡片或第二个同名对象出现。
从业务关系推导出的结果、价值或状态可以用图形表达，但不得新增脚本未提供的结果分类文字、标签或结论。
不得自行新增“设计目标”“核心价值”“预期成效”“总结”等归纳区块、标题或标签；脚本未提供的结果或价值只能用无文字图形状态表达。
默认辅助标签白名单为空；不得从可靠脚本自动提炼或概括新短标签，不得增加节点名、阶段名、输入输出词、状态词、底座名、图例、图标说明或侧边标签。拉丁字母、英文缩写、单字母节点和仿界面令牌同样禁止。
不得虚构日期、版本号、编号、ID、追踪码、状态值、样例数据、界面记录或任何看似真实的示例。
除非内容本身确为并列关系，否则不得把模块处理成等权分栏、卡片墙或重复面板；模块位置、大小、视觉重量和阅读顺序必须由主线决定。
...
不得逐项配图、逐项转成图标或为每个正文模块生成一张图片；小型语义图不得成为独立照片栏，也不得取代整页图形主线。
整页优先形成一个克制而明确的编辑式设计动作，由一条连续形态或一个非对称关系场完成环绕、递进、汇聚或托底；不得用中央徽章、中心大图标、标准圆环、同心台阶、连续箭头胶囊或图标节点链充当视觉主线。图标只能作为低对比微型注释。
脚本已有的关系或业务含义只表达一次，不得在底部或侧边再复制缩略流程、图标链、总结栏或重复关系图。照片只有直接解释具体业务对象时才使用，不得以办公室、城市、园区、机房或泛科技场景填补留白。
视觉主角必须是模块之间的关系，不得把盾牌、锁、数据库、云、芯片、人物或图表放大成中央主视觉。抽象的系统、治理、安全、架构和运行机制页面默认使用纯编辑式图形语言，不使用照片、仿真界面或仿 UI 数据；保持安静留白、精确细线、有限深蓝色块和近乎无阴影的平面质感。
