...
将完整页面设计为一个图文融合的阅读场：图形形态直接承载流程、层级、路径、聚合、分支、优先级和因果关系，使文字产生明确的阅读运动。
关系图形应具有比普通方框和通用连接线更强的视觉表现力，不得退化成原生办公软件风格的简单框线图。
优先将复杂关系收纳在一个或少量边界清晰的矩形关系图容器内；容器可以是界面、图表、业务关系图、教材、文件或设备画面。
矩形关系图内部的节点说明、界面标签、图表刻度、局部关系标注和其他自然文字属于图像表达的一部分，必须清晰、完整，并与容器内图形共同保留。
页面级标题、正文、编号、关键数字、外围标签和结论位于关系图容器之外，与对应矩形图稳定相邻；必须清晰可读，并位于低纹理、对比充分的文字承载面上。
关系图容器不得覆盖或侵入外围正文安全区；外围图形也不得穿过字形。
不得拆成半屏文字加半屏图片，不得生成等权卡片墙或固定分栏；视觉载体的数量、位置、尺寸和形态由本页逻辑决定。"""
