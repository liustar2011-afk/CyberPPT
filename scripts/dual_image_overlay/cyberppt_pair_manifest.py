...
【关系型图形禁用规则｜不上屏】
不得生成流程图、架构图、系统拓扑图、关系网络图、泳道图、分层框架图、节点连线图或其他使用连线表达关系的示意图。
不得生成模块框、流程节点、接口框、标签槽、图标卡片、连接线、方向箭头、分支、回路和层级框线；不得用多个图标、方框或线条拼成系统结构。
仅生成边界清晰的独立语义视觉，例如单一设备、工作物件、业务材料、局部工作场景或单一界面画面。每个插图只表达一个具体对象，不表现多个对象之间的流程、层级或因果关系。
如果页面内容涉及流程、架构、接口协同或多对象关系，在对应位置保留干净、完整、低纹理的浅色留白区域。留白区域内不得生成文字、图标、方框、线条、箭头、装饰纹理或伪界面。"""
