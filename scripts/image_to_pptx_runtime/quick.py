...
def create_quick_project(root: Path | str, *, pages: list[tuple[int, Path]], text_by_page: Mapping[int, Iterable[str]]) -> QuickProject:
    ...
    for number, raw_path in pages:
        ...
        # Keep the runtime archive path short.  The production manifest and
        # inventory already retain the audited source path and SHA-256; using
        # the long page title in this derived filename can exceed Windows'
        # path limit before authored SVG processing even starts.
        archived = sources / f"p{number:02d}-source{source.suffix.lower() or '.bin'}"
