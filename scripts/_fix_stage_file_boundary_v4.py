...
# The Stage2 input module is byte-bound only. Do not invoke Stage1 semantic
# digest/parsing helpers from the cross-stage boundary.
...
text = text.replace(
    ...
    '                if binding.get("sha256") and binding.get("sha256") != _sha256(legacy_script): '
    'raise ValueError("Stage 02 script input changed; rebuild Stage 02 visual artifacts from the updated file")\n'
    '                return legacy_script',
)
text = text.replace(
    'assert "parse_script_path" not in (repo/"cyberppt/stage02_input.py").read_text(encoding="utf-8")',
    'assert "parse_script_path" not in (repo/"cyberppt/stage02_input.py").read_text(encoding="utf-8"); '
    'assert "script_semantic_digest" not in text',
)

append = r"""

# Formal Stage2 freshness is based only on the Stage2-owned input bytes.
...
# Test migration: old tests must not enforce a retired cross-stage handoff.
