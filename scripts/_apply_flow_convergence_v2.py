...
def ensure_replace(rel: str, old: str, new: str) -> None:
    text = read(rel)
    if new in text:
        return
    if old not in text:
        raise RuntimeError(f"{rel}: replacement anchor missing\n--- old ---\n{old}")
...
def replace_block(rel: str, start: str, end: str, block: str) -> None:
    ...
    if start_at < 0:
        raise RuntimeError(f"{rel}: start marker missing: {start}")
    end_at = text.find(end, start_at)
    if end_at < 0:
        raise RuntimeError(f"{rel}: end marker missing: {end}")
    write(rel, text[:start_at] + block + text[end_at:])


# Restore the self-read-default regression without weakening the density gate.
ensure_replace(
...
# Existing branch commits already moved content_load onto each slide. Add the
# default annotation and preserve that corrected location.
schema_path = p("contracts/final-script.schema.json")
schema = json.loads(schema_path.read_text(encoding="utf-8"))
slides = schema["properties"]["slides"]
slides.pop("content_load", None)
load_schema = slides["items"]["properties"].setdefault(
    "content_load", {"type": "string", "enum": ["light", "standard", "dense"]}
)
load_schema["default"] = "standard"
schema_path.write_text(json.dumps(schema, ensure_ascii=False, indent=2) + "\n", encoding="utf-8", newline="\n")

# Markdown is the Stage01 authority. Parse its 内容负载 field into ScriptPage;
# ScriptPage already validates the enum on this work branch.
ensure_replace(
...
# Stage02 verifier validates Stage01 semantics and derives render topology. It
# must never replace Stage01 business relationships in the visual input.
replace_block(
...
# Visual structure receives load and layout guidance separately from semantic
# authority, and reuses an already-current handoff by default.
ensure_replace(
...
# Bind the audited full image to editable reconstruction. This applies only to
# editable/both assembly; picture PPT remains unaffected.
ensure_replace(
...
# Regression coverage for the cross-stage contracts.
write(
...
# Documentation and skill contracts follow the code boundaries.
ensure_replace(
...
# Restore normal CI. The bootstrap workflow disappears in the validated commit.
write(
    ".github/workflows/tests.yml",
    '''name: CyberPPT tests\n\non:\n  pull_request:\n  push:\n    branches:\n      - main\n\njobs:\n  test:\n    runs-on: ubuntu-latest\n    strategy:\n      fail-fast: false\n      matrix:\n        python-version: ["3.10", "3.12"]\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-python@v5\n        with:\n          python-version: ${{ matrix.python-version }}\n          cache: pip\n      - name: Install package and test dependencies\n        run: |\n          python -m pip install --upgrade pip\n          python -m pip install -e ".[test]"\n      - name: Environment check\n        run: |\n          python -m pip check\n          python -m pytest --version\n      - name: Run test suite\n        run: python -m pytest -q\n''',
)
for helper in (p("scripts/_apply_flow_convergence.py"), Path(__file__)):
    if helper.exists():
        helper.unlink()
