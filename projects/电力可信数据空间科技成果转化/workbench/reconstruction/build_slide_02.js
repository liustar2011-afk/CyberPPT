...
const LAYOUT = require("./layout_context");
...
// Main matrix
const BODY = LAYOUT.safe_body_zone;
const SO_WHAT = LAYOUT.so_what_band;
const matrixX = BODY.x + 0.08;
const matrixY = BODY.y + 0.20;
const labelW = 1.30;
const colW = 1.94;
const colGap = 0.04;
const rowH = 1.36;
...
// Right policy panel
const px = BODY.x + BODY.w - 2.70;
rect(px, matrixY - 0.06, 2.48, 0.34, C.accent, C.accent);
addText("政策依据（源文件列明）", px, matrixY + 0.02, 2.48, 0.18, { fontSize: 10.5, bold: true, color: C.white, align: "center" });
const policies = [
...
addText("注：以上为源文件明确列示依据；具体条文和外部数据可在立项材料中进一步补充。", px, BODY.y + BODY.h - 0.28, 2.44, 0.42, { fontSize: 6.3, color: C.secondary });

// SO WHAT band
const sy = SO_WHAT.y;
const sh = SO_WHAT.h;
rect(SO_WHAT.x, sy, 1.32, sh, C.accent, C.accent);
addText("SO WHAT", SO_WHAT.x + 0.12, sy + sh / 2 - 0.09, 0.86, 0.18, { fontSize: 11, bold: true, color: C.white, align: "center" });
const implications = [
...
implications.forEach((it, i) => {
  const x = SO_WHAT.x + 1.48 + i * 2.22;
  rect(x, sy, 2.05, sh, "F9FAF7", C.line, 0.5);
  addText(it[0], x + 0.12, sy + 0.10, 0.55, 0.18, { fontSize: 8, bold: true, color: C.accent });
  addText(it[1], x + 0.12, sy + 0.31, 1.72, 0.18, { fontSize: 7.2, color: C.body });
});
rect(SO_WHAT.x + SO_WHAT.w - 4.20, sy, 4.00, sh, C.accent, C.accent);
addText("项目具备内部立项的政策与职能基础", SO_WHAT.x + SO_WHAT.w - 3.90, sy + sh / 2 - 0.15, 3.40, 0.30, { fontSize: 14, bold: true, color: C.white, align: "center" });
