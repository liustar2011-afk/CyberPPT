...
External source files are always copied into the project. `--move` is honored
only for files already inside this repository, where the source is usually a
temporary conversion artifact.
"""
...
class ProjectManager:
    ...
    def import_sources(
        ...
        for item in source_items:
            ...
            source_inside_repo = is_within_path(source_path, REPO_ROOT)
            if copy:
                ...
                if source_inside_repo:
                    ...
                    summary["notes"].append(
                        f"{item}: copied because external source files must not be moved"
                    )
            elif source_inside_repo:
                effective_move = True
                print(
