...
from .drawingml_utils import (
    SVG_NS, EMU_PER_PX,
    _extract_inheritable_styles, parse_transform_matrix, resolve_url_id, _xml_escape,
)
...
from .semantic_names import has_semantic_name_source, semantic_shape_name, uniquify_selection_name
...
def convert_g(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    should_preserve_semantic_group = has_semantic_name_source(elem)
    visual_children = [
    ...
    if len(child_results) == 1 and not should_animate_group and not should_preserve_semantic_group:
    ...
    group_name = uniquify_selection_name(
        semantic_shape_name(elem, 'group', group_id, page_id=ctx.page_id),
        ctx.selection_name_counts,
    )

    return ShapeResult(xml=f'''<p:grpSp>
<p:nvGrpSpPr>
<p:cNvPr id="{group_id}" name="{_xml_escape(group_name)}"/>
<p:cNvGrpSpPr/>
<p:nvPr/>
</p:nvGrpSpPr>
<p:grpSpPr>
...
def convert_svg_to_slide_shapes(
    ...
    try:
        from shared_ppt_resources import icons_dir as resolve_shared_icons_dir
        icons_dir = resolve_shared_icons_dir()
    except ImportError:
