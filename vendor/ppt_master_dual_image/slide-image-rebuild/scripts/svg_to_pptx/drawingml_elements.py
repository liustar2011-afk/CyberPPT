...
from .semantic_names import semantic_shape_name, uniquify_selection_name


def _selection_name(elem: ET.Element, fallback_kind: str, shape_id: int, ctx: ConvertContext) -> str:
    name = semantic_shape_name(elem, fallback_kind, shape_id, page_id=ctx.page_id)
    return uniquify_selection_name(name, ctx.selection_name_counts)
...
def convert_rect(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _selection_name(elem, 'rect', shape_id, ctx),
...
def convert_circle(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if _is_donut_circle(elem, ctx):
        ...
        return ShapeResult(
            xml=_wrap_shape(
                shape_id, _selection_name(elem, 'arc', shape_id, ctx),
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _selection_name(elem, 'ellipse', shape_id, ctx),
            off_x, off_y, ext_cx, ext_cy,
...
def convert_line(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if has_marker:
        ...
        rot_attr = f' rot="{rot}"' if rot else ''
        name = _selection_name(elem, 'line', shape_id, ctx)
        xml = (
            f'<p:sp>'
            f'<p:nvSpPr>'
        ...
        xml = _wrap_shape(
            shape_id, _selection_name(elem, 'line', shape_id, ctx),
...
{path_xml}
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _selection_name(elem, 'path', shape_id, ctx),
            off_x, off_y, w_emu, h_emu,
            geom, fill, stroke, effect, rot=rot,
        ),
        bounds_emu=(off_x, off_y, off_x + w_emu, off_y + h_emu),
...
{path_xml}
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _selection_name(elem, 'polygon', shape_id, ctx),
            off_x, off_y, w_emu, h_emu,
...
{path_xml}
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _selection_name(elem, 'polyline', shape_id, ctx),
            off_x, off_y, w_emu, h_emu,
            geom, '<a:noFill/>', stroke, rot=rot,
        ),
        bounds_emu=(off_x, off_y, off_x + w_emu, off_y + h_emu),
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    shape_name = _selection_name(elem, 'text', shape_id, ctx)
...
def convert_image(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    shape_name = _selection_name(elem, 'image', shape_id, ctx)
