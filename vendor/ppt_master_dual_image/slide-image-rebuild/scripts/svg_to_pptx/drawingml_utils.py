...
# Common SVG/CSS named colors. Strict rebuild QA still requires authored SVGs to
# use #RRGGBB, but the converter normalizes these as a last-resort guard so a
# skipped QA step cannot silently turn "white" text into black in PowerPoint.
SVG_NAMED_COLORS = {
    'black': '000000',
    'silver': 'C0C0C0',
    'gray': '808080',
...
def parse_hex_color(color_str: str) -> str | None:
    """Parse SVG color values to DrawingML 'RRGGBB'.

    Authoring rules still prefer explicit hex colors; this parser accepts
    common named colors and rgb()/rgba() as a converter safety net.
    """
    if not color_str:
        return None
    color_str = color_str.strip()
    lowered = color_str.lower()
    if lowered in SVG_NAMED_COLORS:
        return SVG_NAMED_COLORS[lowered]
    ...
    if rgb_match:
        ...
        if len(parts) >= 3:
            ...
            for part in parts[:3]:
                try:
                    if part.endswith('%'):
                        value = round(float(part[:-1]) * 2.55)
                    else:
                        value = round(float(part))
                except ValueError:
                    return None
