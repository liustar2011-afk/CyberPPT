...
def copy_text_attrs(
    ...
    for k in TEXT_STYLE_ATTRS:
        ...
        if v is not None:
            dst_el.set(k, v)
    # Custom data-* markers (data-text-region-id, data-fit-*, data-paragraph-line-height,
    # ...) identify which logical text region/paragraph an element belongs to. Flattening
    # splits one <text> into several -- every resulting line is still part of the same
    # region, so these markers must follow onto each flattened line, not just be dropped.
    for k, v in src_el.attrib.items():
        if k.startswith("data-") and k not in exclude:
            dst_el.set(k, v)
