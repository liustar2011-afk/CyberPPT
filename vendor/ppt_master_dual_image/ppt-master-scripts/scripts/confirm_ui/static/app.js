...
(function () {
    ...
    function submitTier1() {
        ...
        if (REC.core_thesis && REC.core_thesis.applicable !== false) {
            if (!String(STATE.core_thesis || "").trim()) {
                document.getElementById("confirm-status").textContent = t("thesis_required");
                var tier1ThesisInput = document.querySelector(".core-thesis-custom-input");
                if (tier1ThesisInput && tier1ThesisInput.style.display !== "none") tier1ThesisInput.focus();
                return;
            }
            payload.core_thesis = STATE.core_thesis;
        }
