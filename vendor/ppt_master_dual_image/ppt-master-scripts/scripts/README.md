...
| SVG pipeline | `finalize_svg.py`, `svg_to_pptx.py`, `total_md_split.py`, `svg_quality_checker.py`, `page_content_checker.py`, `import_ppt_arrow_pack_assets.py`, `verify_arrow_templates.py`, `extract_svg_assets.py`, `animation_config.py`, `notes_to_audio.py` | [docs/svg-pipeline.md](./docs/svg-pipeline.md) |
...
| Script imagegen to PPT | `page_image_pair_batch.py`, `template_image_ppt_export.py`, `script_imagegen_rebuild_template.py`, `ocr_text_locator.py`, `script_text_overlay.py` | `page_image_pair_batch.py run` exports image-based PPTX. `script_imagegen_rebuild_template.py run` exports no-text background + editable text overlay PPTX. |
...
python3 scripts/import_ppt_arrow_pack_assets.py
python3 scripts/verify_arrow_templates.py
...
python3 scripts/template_image_ppt_export.py plan --script <script.md> --pages 1-5 -o <out> --image-style 象牙白深蓝强调
python3 scripts/page_image_pair_batch.py plan --script <script.md> --pages 1-5 -o <out> --image-style 象牙白深蓝强调
python3 scripts/page_image_pair_batch.py run --script <script.md> --pages 12-14 --project-name <name> --parallel-pages 3
python3 scripts/page_image_pair_batch.py run --script <script.md> --pages 12-14 --project-path <project> --resume --parallel-pages 3 --draft
python3 scripts/page_image_pair_batch.py run --script <script.md> --pages 12-14 --project-name <name> --dual-image
python3 scripts/script_imagegen_rebuild_template.py run --script <script.md> --pages 12-14 --project-name <name> --parallel-pages 3 --ocr-backend vision-json
python3 scripts/script_imagegen_rebuild_template.py rebuild <project>/images/script_imagegen/page_image_pairs.json --ocr-backend vision-json --export
```

Script-image workflows load reusable prompt style presets from
`templates/image_styles/`. `--image-style` accepts either a preset name or a
JSON/Markdown style file path; when omitted, the default is `象牙白深蓝强调`
(`象牙白 + 深蓝强调`, migrated from CyberPPT option 04). The previous
`中电联政企清爽专业风` preset remains available by passing it explicitly.
For `page_image_pair_batch.py`, the default is full-image-only: it generates and
exports one full content image per script page. Use `--dual-image` only when
background images are explicitly required; in that mode each page preserves the
internal `full -> background` order.
`--parallel-pages N` runs independent pages concurrently.
`--draft` switches default speed settings to medium quality, `1x-content-region`
image-size, and a 1x content-region canvas unless those options are explicitly set.
`--resume` reuses an existing
`page_image_pairs.json` in the image directory and keeps generated image files as
cache; add `--force` only when existing images should be overwritten.
For editable-text overlay, `script_imagegen_rebuild_template.py` internally uses
full/background pairs: the full image supplies OCR text coordinates, the script
supplies final text truth, and the background image becomes the bottom visual
asset under editable PPT text boxes.
