...
> When pages map to a visualization template (data charts, structural patterns, or arrow templates), Strategist lists them here for Executor reference. Single combined table — `summary-quote` column is the anti-fabrication audit, `path` + `usage` columns serve Executor lookup.
> For pages whose main visual is an arrow relationship, prefer indexed arrow templates before drawing from scratch. For connector-heavy pages, search `connector_index.json` first; for broad structural arrows, search `arrows_index.json`. Prefer `pptpack7_orthogonal_connector_*`, `pptpack7_hub_exchange_topology_*`, `pptpack7_cycle_feedback_*`, `pptpack5_node_link_connector_*`, `pptpack*_gradient_*`, `pptpack4_connector_route_*`, or other structural arrow templates for architecture grammar, process flow, action, cycle, and relation emphasis. Do not select icon-library-derived arrows as primary slide visuals.

Catalog read: [charts count] chart templates + [arrows count] arrow templates + [connector count] connector templates

| Page | Template | Path | Summary-quote (verbatim from `charts_index.json` or `arrows_index.json`) | Usage |
| ---- | -------- | ---- | ------------------------------------------------- | ----- |
...
| P08 | block_chevron_sequence | `templates/arrows/block_chevron_sequence.svg` | "Pick for 3-6 horizontal phase chains where each stage is a chunky chevron block with editable labels. Skip for plain node-to-node workflow (use connector_arrow_set or process_flow)." | Five-stage implementation path |
