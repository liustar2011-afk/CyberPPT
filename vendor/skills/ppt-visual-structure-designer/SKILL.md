...
读取 canonical intake `workbench/stages/02-input/script-intake.json` 中目标页的完整语义、来源信息及必要的相邻页。Final Script 1.2 使用完整 `full_prose`（来源 full_copy）和适配后 `content_text`；外部稿使用适配后的结构化内容或自由正文。来源引用可用于回读精确原文；无法获得原文时在 `source_check` 明确本次仅核对讲稿，不能声称已核验原始资料。
