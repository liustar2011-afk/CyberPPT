...
description: 将PPT原始内容、逐页脚本或既有页面方案转化为可执行的视觉结构设计规格与整页生图脚本。用于PPT页面使命判断、核心结论提炼、语义关系建模、视觉意图选择、主视觉载体设计、实景插图与文字融合、页面构图重构、单页修复、整套视觉节奏设计和视觉脚本质量审查。适用于“设计PPT脚本的视觉结构”“重构页面构图”“解决图是图文字是文字”“按内容决定页面怎么画”“形成Codex生图脚本”等任务；不直接生成图片、HTML、SVG或PPTX。
---
...
- 为每页选择视觉意图类型、主视觉载体、实景锚点、图文融合方式和禁止事项。
...
- 允许调整的换行、分组、标注位置、视觉载体和空间顺序。
- 比例、字体、字号、配色、标题渲染方式、公共模板元素和禁用项。
...
每页内部生成至少三种结构上真正不同的候选。候选必须改变主视觉载体、空间关系或阅读路径，不能只换颜色或模块顺序。
...
生成模块必须先给构图指令，再给上屏文字和风格要求；字段名和指令文字不得成为画面文字。
...
- 16:9，参考画布1280×720。
- 象牙白或白底，深蓝为主色，低饱和科技蓝为辅助色。
- 使用Microsoft YaHei或视觉接近微软雅黑的中文无衬线字体。
- 标题和副标题默认由PPT文字层处理，整页生图不绘制标题区文字。
- 正文字号不得通过压缩到14pt以下解决容量问题。
- 优先采用具有业务语义的实景彩色插图、场景化空间或真实行业对象。
- 实景图必须参与表达关系，禁止成为与文字分离的装饰图片。
- 不出现正面人物形象；必要人物只使用背影、侧影或非特定身份工作场景。
- 允许具有表现力的行业场景，但不得暗示具体地点、具体事件或事实照片。
- 箭头和关系线保持干净、方向一致，不穿越文字或关键数字。
- 不绘制Logo、页码、顶部红蓝条和固定模板装饰。
