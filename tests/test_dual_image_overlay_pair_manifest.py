...
class CyberpptPairManifestTests(unittest.TestCase):
    def test_dual_image_full_prompt_uses_graphics_to_carry_text_relationships(self) -> None:
        ...
        self.assertIn("这套跨页面展开的图形构图是主要组织层", prompt)
        self.assertIn("不受插图矩形容器限制", prompt)
        self.assertIn("主体、支撑、输入、输出、分支、汇聚、闭环、层级、对比、因果与结论", prompt)
        self.assertIn("不得仅按文本顺序机械排列或连接", prompt)
        self.assertIn("每个锁定模块及其名称只能承担一个主要关系角色并出现一次", prompt)
        self.assertIn("不得再作为外围节点、关系标签、重复卡片或第二个同名对象出现", prompt)
        self.assertIn("不得新增脚本未提供的结果分类文字、标签或结论", prompt)
        self.assertIn("不得把模块处理成等权分栏、卡片墙或重复面板", prompt)
        ...
        self.assertIn("不得逐项配图、逐项转成图标", prompt)
