...
from cyberppt.outline_audit_semantics import _structural_argument_duty_issues
...
class OutlineContractTests(unittest.TestCase):
    ...
    def test_required_consumption_uses_page_scoped_unit_duty(self) -> None:
        content = page(
            1,
            "content",
            "首期投入",
        ...
        content.update({
            ...
            "content_units": [{
                "unit_id": "p01-u01",
                "statement": "首期投入控制在10—20万元以内",
                "source_refs": ["S001"],
                "argument_duties": ["support"],
                "onscreen_required": False,
            }],
        })
        truth = {"records": [{"id": "S001", "argument_duty": "response"}]}

        self.assertEqual([], _structural_argument_duty_issues([content], truth))
