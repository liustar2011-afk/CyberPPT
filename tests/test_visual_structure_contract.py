...
def test_audit_rejects_cross_root_grouping() -> None:
    design, decisions, spec = _payloads()
    spec["pages"][0]["content_integrity"] = {
        "nodes": [
            {"text_id": "P01-T01", "root_id": "P01-T01", "content_role": "root_module"},
            {"text_id": "P01-T02", "root_id": "P01-T02", "content_role": "root_module"},
        ]
    }
    spec["pages"][0]["semantic_graph"]["grouping_decisions"] = [
        {"source_nodes": ["P01-T01", "P01-T02"], "target_node": "E1"}
    ]
    assert "CONTENT_CROSS_ROOT_GROUPING" in {
        item["code"] for item in _audit(design, decisions, spec)["blocking_issues"]
    }


def test_audit_rejects_detail_only_focus_promotion() -> None:
    design, decisions, spec = _payloads()
    spec["pages"][0]["content_integrity"] = {
        "nodes": [
            {"text_id": "P01-T01", "root_id": "P01-T01", "content_role": "root_module"},
            {"text_id": "P01-T02", "root_id": "P01-T01", "content_role": "detail"},
        ]
    }
    spec["pages"][0]["semantic_graph"]["focus_node"] = "E2"
    assert "CONTENT_FOCUS_PROMOTION" in {
        item["code"] for item in _audit(design, decisions, spec)["blocking_issues"]
    }


def test_audit_accepts_root_module_focus_with_valid_content_integrity() -> None:
    design, decisions, spec = _payloads()
    spec["pages"][0]["content_integrity"] = {
        "nodes": [
            {"text_id": "P01-T01", "root_id": "P01-T01", "content_role": "root_module"},
    ...
    spec["pages"][0]["semantic_graph"]["focus_node"] = "E2"
    codes = {item["code"] for item in _audit(design, decisions, spec)["blocking_issues"]}
    assert "CONTENT_CROSS_ROOT_GROUPING" not in codes
    assert "CONTENT_FOCUS_PROMOTION" not in codes


def test_audit_has_no_semantic_edge_warning_on_baseline() -> None:
    design, decisions, spec = _payloads()
    report = _audit(design, decisions, spec)
    codes = {item["code"] for item in report["warnings"]}
    assert "SEMANTIC_EDGE_UNGROUNDED" not in codes


def test_audit_warns_on_ungrounded_directional_edges() -> None:
    design, decisions, spec = _payloads()
    design["pages"][0]["business_relationships"] = []
    spec["pages"][0]["semantic_graph"]["business_relationships"] = []
    spec["pages"][0]["semantic_graph"]["edges"] = [
        {"from": "E1", "to": "E2", "relation": "flow", "label": "业务承接", "direction": "forward"}
    ]
    report = _audit(design, decisions, spec)

    codes = {item["code"] for item in report["warnings"]}
    assert "SEMANTIC_EDGE_UNGROUNDED" in codes
    # This is a visibility signal only -- it must never fail the audit.
    assert report["status"] == "passed"
    assert "SEMANTIC_EDGE_UNGROUNDED" not in {item["code"] for item in report["blocking_issues"]}


def test_audit_does_not_warn_when_edges_have_business_relationships() -> None:
    design, decisions, spec = _payloads()
    spec["pages"][0]["semantic_graph"]["edges"] = [
        {"from": "E1", "to": "E2", "relation": "flow", "label": "业务承接", "direction": "forward"}
    ]
...
def test_audit_does_not_warn_on_ungrounded_peer_edges() -> None:
    design, decisions, spec = _payloads()
    design["pages"][0]["business_relationships"] = []
    spec["pages"][0]["semantic_graph"]["business_relationships"] = []
    spec["pages"][0]["semantic_graph"]["edges"] = [
        {"from": "E1", "to": "E2", "relation": "peer", "label": "共同支撑同一判断", "direction": "none"}
    ]
    report = _audit(design, decisions, spec)
    codes = {item["code"] for item in report["warnings"]}
    assert "SEMANTIC_EDGE_UNGROUNDED" not in codes
