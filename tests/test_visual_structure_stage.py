...
class VisualStructureStageTests(unittest.TestCase):
    ...
    def _content_integrity_source(self) -> tuple[list[dict[str, str]], dict[str, object]]:
        """One root module with two details, plus a second, unrelated root.

        Mirrors the architecture plan's P05 negative example: a detail line
        must not silently become root-eligible just because it gets merged
        or picked as the visual focus.
        """

        page = ScriptPage(
            page_id="p01", sequence=1, heading="", page_type="content", title="",
            main_message="", full_prose="", selection_notes="", evidence_map="",
            evidence_map_refs=(), source_refs=(), boundary_source_refs=(), boundary="",
            visual_structure="",
            onscreen_text="Root text\n  Detail one\n  Detail two\nSecond root",
            module_titles=(),
        )
        contract = build_content_integrity_contract(page).to_dict()
        locked_text_items = [{"text_id": node["text_id"], "text": node["text"]} for node in contract["nodes"]]
        return locked_text_items, contract

    def _content_integrity_decision(self, evidence_units: list[dict[str, object]], focus_key: str) -> dict[str, object]:
        return {
            "page_id": "p01",
            ...
            "candidates": [
                {
                    "id": "c1",
                    "semantic_focus": {"kind": "outcome", "evidence_key": focus_key},
                    "reading_sequence": [item["key"] for item in evidence_units],
                    "spatial_grammar": ["convergence"],
                    "topology": "causal_convergence",
                    "direction": "outside_to_anchor",
                    "visual_intent_type": "input_to_result",
                    "expression_fit": {
                        ...
                        "constraint_status": "default_profile",
                        "satisfied_constraints": ["ordered_progression"],
                        "reading_relation": "detail supports the root module",
                        "balance_strategy": "one dominant result",
                        "changed_constraints": [],
                        "deviation_reason": "",
                    },
            ...
            "selected_candidate": "c1",
            "execution_design": {
                "business_object": "root module in an operating environment",
                "visual_focus": "the root module the details support",
                "text_integration_method": "attach each locked phrase to its related module",
                "spatial_organization": "details converge on the root module",
                "relationship_encoding": "convergence encodes support",
                "semantic_role": "the root module carries the page result",
                "use_scene": False,
                "scene_type": "不使用实景，由选定业务关系场承载",
            },
        }

    def _content_integrity_source_page(self, locked_text_items: list[dict[str, str]], contract: dict[str, object]) -> dict[str, object]:
        return {
            "page_id": "p01",
            "page_number": 1,
            "page_title": "Title",
            "page_mission": "Explain how the details support the root module.",
            "core_judgment": "The root module carries the page result.",
            "locked_text_items": locked_text_items,
            "content_integrity": contract,
            "business_relationships": [],
            "expression_constraints": expression_constraints("flow_3_5"),
            "body_image_canvas": {"width": 2048, "height": 1024, "ratio": "2:1"},
            "title_render_mode": "external_text_layer",
    ...
    def test_same_root_grouping_and_root_module_focus_compiles(self) -> None:
        locked, contract = self._content_integrity_source()
        source = self._content_integrity_source_page(locked, contract)
        evidence_units = [
            {
                "key": "root_group", "summary": "Root text with its details",
                "text_ids": ["P01-T01", "P01-T02", "P01-T03"],
                "grouping_reason": "same root module", "loss_risk": "low",
            },
            {"key": "second", "summary": "Second root", "text_ids": ["P01-T04"]},
        ]
        decision = self._content_integrity_decision(evidence_units, focus_key="root_group")

        page = _build_executable_page(source, decision)

        self.assertEqual("E1", page["semantic_graph"]["focus_node"])

    def test_cross_root_grouping_is_rejected(self) -> None:
        locked, contract = self._content_integrity_source()
        source = self._content_integrity_source_page(locked, contract)
        evidence_units = [
            {"key": "a", "summary": "Root and its first detail", "text_ids": ["P01-T01", "P01-T02"]},
            {
                "key": "b", "summary": "Second detail merged with the unrelated second root",
                "text_ids": ["P01-T03", "P01-T04"],
                "grouping_reason": "adjacent in reading order", "loss_risk": "low",
            },
        ]
        decision = self._content_integrity_decision(evidence_units, focus_key="a")

        with self.assertRaisesRegex(ValueError, "cross-root grouping is forbidden"):
            _build_executable_page(source, decision)

    def test_detail_only_focus_is_rejected(self) -> None:
        locked, contract = self._content_integrity_source()
        source = self._content_integrity_source_page(locked, contract)
        evidence_units = [
            {"key": "root_alone", "summary": "Root text", "text_ids": ["P01-T01"]},
            {"key": "detail_alone", "summary": "Detail one", "text_ids": ["P01-T02"]},
            {"key": "detail_two", "summary": "Detail two", "text_ids": ["P01-T03"]},
            {"key": "second_root", "summary": "Second root", "text_ids": ["P01-T04"]},
        ]
        decision = self._content_integrity_decision(evidence_units, focus_key="detail_alone")

        with self.assertRaisesRegex(ValueError, "detail-only evidence unit cannot become the page result/judgment"):
            _build_executable_page(source, decision)

    def test_root_module_alone_can_be_focus(self) -> None:
        locked, contract = self._content_integrity_source()
        source = self._content_integrity_source_page(locked, contract)
        evidence_units = [
            {"key": "root_alone", "summary": "Root text", "text_ids": ["P01-T01"]},
        ...
        decision = self._content_integrity_decision(evidence_units, focus_key="root_alone")
