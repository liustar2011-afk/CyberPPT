...
class PageArtifactSpecTests(unittest.TestCase):
    ...
    def test_evidence_root_id_is_empty_without_content_integrity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            handoff_page, visual_page, style_lock = self._inputs(root)
        ...
        self.assertEqual(("", ""), tuple(item.root_id for item in spec.evidence))
        self.assertEqual(0, spec.content_root_count)

    def test_evidence_root_id_resolves_from_content_integrity_and_text_bindings(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            handoff_page, visual_page, style_lock = self._inputs(root)
            visual_page = deepcopy(visual_page)
            visual_page["content_integrity"] = {
                "schema": "cyberppt.content_integrity.v1",
                "root_nodes": ["P07-T01", "P07-T04"],
                "nodes": [
                    {"text_id": "P07-T01", "root_id": "P07-T01", "content_role": "root_module"},
                    {"text_id": "P07-T02", "root_id": "P07-T04", "content_role": "root_module"},
                ],
            }
            visual_page["structural_decision"]["text_bindings"] = [
                {"evidence_id": "E1", "target_ref": "E1", "binding": "embedded", "text_ids": ["P07-T01"]},
                {"evidence_id": "E2", "target_ref": "E2", "binding": "result", "text_ids": ["P07-T02"]},
            ]
        ...
        by_summary = {item.summary: item for item in spec.evidence}
        self.assertEqual("P07-T01", by_summary["Governed input enters"].root_id)
        self.assertEqual("P07-T04", by_summary["Traceable result emerges"].root_id)
        self.assertEqual(2, spec.content_root_count)
