...
class FinalPromptIRTests(unittest.TestCase):
    ...
    def test_rejects_more_than_max_semantic_groups(self) -> None:
        groups = tuple(_group(id_=f"kind{i}", emphasis="primary" if i == 0 else "secondary") for i in range(MAX_SEMANTIC_GROUPS + 1))
        with self.assertRaisesRegex(PromptContractError, f"at most {MAX_SEMANTIC_GROUPS} semantic groups"):
            _ir(semantic_groups=groups)

    def test_accepts_up_to_max_semantic_groups(self) -> None:
...
class BuildFinalPromptIRTests(unittest.TestCase):
    def test_humanizes_visual_structure_enums_before_prompt_rendering(self) -> None:
        spec = replace(
            _artifact_spec(),
            visual_thesis="画面以support_convergence关系组织证据。",
            composition=replace(
                _artifact_spec().composition,
                spatial_organization="以support_convergence关系组织证据单元。",
            ),
        )
        ...
        rendered_fields = " ".join(
            (
                ...
                ir.composition.spatial_organization,
                *ir.composition.visual_responsibility,
            )
        )
        self.assertIn("multiple evidence lines converging on one judgment", rendered_fields)
        self.assertNotIn("support_convergence", rendered_fields)
    ...
    def test_semantic_groups_exceeding_declared_root_count_raises_capacity_error(self) -> None:
        spec = replace(
            _artifact_spec(evidence_kinds=("process", "result", "extra")),
            content_root_count=2,
        )
        with self.assertRaisesRegex(
            PromptContractError, r"P05:.*CONTENT_STRUCTURE_CAPACITY_EXCEEDED.*\(3\).*2 authoritative root modules"
        ):
            build_final_prompt_ir(spec)

    def test_semantic_groups_matching_declared_root_count_does_not_raise(self) -> None:
        spec = replace(_artifact_spec(), content_root_count=2)
        ir = build_final_prompt_ir(spec)
        self.assertEqual(2, len(ir.semantic_groups))

    def test_zero_content_root_count_falls_back_to_static_ceiling(self) -> None:
        # content_root_count=0 (the dataclass default) means "unknown/legacy
        # page" -- the dynamic check must not fire even when the group count
        # would exceed a small number; only the static MAX_SEMANTIC_GROUPS
        # ceiling (tested separately) still applies.
        spec = replace(_artifact_spec(evidence_kinds=("process", "result", "extra")))
        self.assertEqual(0, spec.content_root_count)
        ir = build_final_prompt_ir(spec)
        self.assertEqual(3, len(ir.semantic_groups))

    def test_more_than_max_evidence_kinds_raises_with_page_id(self) -> None:
        kinds = tuple(f"kind{i}" for i in range(MAX_SEMANTIC_GROUPS + 1))
        with self.assertRaisesRegex(PromptContractError, rf"P05:.*at most {MAX_SEMANTIC_GROUPS} semantic groups"):
