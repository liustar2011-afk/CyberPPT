...
""",
        ...
        self.assertIn("【源文案语义输入】", prompt)
        self.assertIn("可将源文案改写为结论、标题、标签、流程节点或正文", prompt)
        self.assertIn("改写并列内容时，保留父级与子项的实际适用范围", prompt)
        self.assertIn("避免把同一内容以正文和拆分标签机械重复呈现", prompt)
        self.assertIn("任何按业务语义选择的视觉载体", prompt)
        self.assertNotIn("大屏、图表、文件、装置或图标可以", prompt)
        self.assertIn("都可承载经过改写的可见文字", prompt)
        ...
        self.assertIn("可使用更清晰的同义表达", prompt)
    ...
    def test_cli_writes_manifest_policy(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [
                    "python3",
                    str(ROOT / "scripts/imagegen_pipeline/deliverable_prompt.py"),
                    "--script",
                    str(script),
                    "--pages",
                    "2",
