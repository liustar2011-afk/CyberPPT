...
from cyberppt.script_quality.source_coverage import _projected_table_header_record_ids
from cyberppt.script_quality.onscreen import _is_structured_compact_onscreen_layer
from cyberppt.script_quality.relationships import _page_relationship_contract_issues
...
class OnscreenModuleIndexTests(unittest.TestCase):
    def _page(
        self,
        text: str,
        modules: tuple[str, ...],
        *,
        visual_intent_type: str = "judgment_evidence",
    ) -> ScriptPage:
        return ScriptPage(
            page_id="p04",
            sequence=4,
            ...
            title="核心结论",
            subtitle="先验证商业闭环",
            main_message="现有能力支持轻量验证。",
            full_prose="现有能力支持轻量验证，并形成可审计的业务判断。",
            selection_notes="",
            evidence_map="",
            evidence_map_refs=(),
            source_refs=(),
    ...
    def test_three_module_index_restatement_and_undeclared_order_are_blocked(self) -> None:
        modules = ("专业能力基础", "训练化转化关系", "轻量验证原则", "近期策略")
        page = self._page(
            "专业能力基础 → 训练化转化关系 → 轻量验证原则 → 近期策略。\n"
            "专业能力基础\n  交易辅助：电力交易辅助决策\n"
            "训练化转化关系\n  转化方向：企业岗位实训\n"
            "轻量验证原则\n  验证对象：付费与交付\n"
            "近期策略\n  付费试点：1—2个真实项目",
        ...
        self.assertIn("ONSCREEN_MODULE_INDEX_RESTATEMENT", codes)
        self.assertIn("ONSCREEN_UNDECLARED_ORDER_SIGNAL", codes)

    def test_two_node_business_arrow_is_not_a_module_index(self) -> None:
        modules = ("数据接入", "模型计算")
        page = self._page(
            "数据接入 → 模型计算\n"
            "数据接入\n  输入：历史交易数据\n"
        ...
        self.assertNotIn("ONSCREEN_MODULE_INDEX_RESTATEMENT", codes)
        self.assertNotIn("ONSCREEN_UNDECLARED_ORDER_SIGNAL", codes)

    def test_explicit_execution_chain_requires_an_order_contract(self) -> None:
        modules = ("合作主体", "实施边界", "交付结果")
        page = self._page(
            "执行链：主体职责 → 投入门控 → 必要改造 → 轻量MVP交付\n"
            "合作主体\n  分工：明确交付责任\n"
            "实施边界\n  范围：完成必要适配\n"
            "交付结果\n  结果：交付验证版本",
            modules,
    ...
    def test_arrow_inside_a_module_is_not_an_order_signal(self) -> None:
        page = self._page(
            "合作主体\n  人才中心 → 负责 → 课程客户\n"
            "交付内容\n  课程服务：岗位训练课程",
            ("合作主体", "交付内容"),
    ...
    def test_top_level_responsibility_relation_is_not_an_order_signal(self) -> None:
        page = self._page(
            "责任关系：人才中心 → 负责 → 课程客户\n"
            "合作主体\n  主体：人才中心\n"
            "服务对象\n  对象：课程客户",
    ...
    def test_explicit_order_label_with_one_arrow_is_not_a_chain(self) -> None:
        page = self._page(
            "执行链：主体职责 → 轻量MVP交付\n"
            "合作主体\n  分工：明确交付责任\n"
            "交付结果\n  结果：交付验证版本",
    ...
    def test_declared_flow_relation_allows_distinct_order_signal(self) -> None:
        modules = ("数据准备", "模型训练", "效果复核")
        page = self._page(
            "阶段链：原始数据 → 训练样本 → 评估结果\n"
            "数据准备\n  输入：原始数据\n"
            "模型训练\n  产出：训练样本\n"
        ...
        contract = {
            "content_relations": [
                {"relation": "flows_to", "source_refs": ["ST001"]},
            ],
        }
    ...
    def test_required_legacy_structure_contract_warns_without_guessing_fields(self) -> None:
        page = self._page(
            "专业能力基础\n  交易辅助：电力交易辅助决策\n"
            "近期策略\n  付费试点：1—2个真实项目",
            ("专业能力基础", "近期策略"),
        ...
        migration = next(
            issue for issue in issues if issue.code == "ONSCREEN_STRUCTURE_CONTRACT_LEGACY"
        )
        self.assertEqual("warning", migration.severity)
        self.assertIn("peer_dimension", migration.suggested_action)
...
class FullProseSourceCoverageTests(unittest.TestCase):
    ...
    def test_explicit_table_header_only_ignores_proven_repeated_label(self) -> None:
        source_units = ({
            "unit_id": "SU-TABLE",
            "kind": "table",
            "locator": {"line_start": 1, "line_end": 4},
            "metadata": {
                "table_header": {
                    "status": "confirmed",
                    "row_index": 1,
                    "line_start": 3,
                    "cells": ["课程/产品", "核心训练内容", "建议价格"],
                }
            },
            "text": (
                "|  |  |  |\n"
                "| --- | --- | --- |\n"
                "| 课程/产品 | 核心训练内容 | 建议价格 |\n"
                "| 交易实训 | 模拟申报与收益复盘 | 15万元/班 |"
            ),
        },)
        records = {
            "ST001": {"statement": "课程/产品", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 3}},
            "ST002": {"statement": "课程/产品：核心训练内容", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 3}},
            "ST003": {"statement": "课程/产品：建议价格", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 3}},
            "ST004": {"statement": "交易实训", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 4}},
            "ST005": {"statement": "交易实训：模拟申报与收益复盘", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 4}},
        }
        metadata_refs = _projected_table_header_record_ids(records, source_units)
        self.assertEqual(frozenset({"ST001", "ST002", "ST003"}), metadata_refs)
        self.assertNotIn("ST004", metadata_refs)

        page = replace(
            self._page("交易实训覆盖模拟申报与收益复盘。"),
            onscreen_text="交易实训\n  训练内容：模拟申报与收益复盘",
        )
        units = [
            {
                "unit_id": f"p10-u{index:02d}",
                "statement": record["statement"],
                "source_refs": [record_id],
                "full_prose_required": True,
                "coverage_anchors": [record["statement"]],
                "onscreen_required": True,
                "onscreen_anchors": [record["statement"]],
            }
        ...
        header_gaps = {
            ...
            if source_id in metadata_refs
        }
        self.assertEqual(set(), header_gaps)

        missing_body = replace(page, onscreen_text="交易实训")
        body_issues = _page_content_unit_coverage_issues(
            missing_body,
            {"content_units": units},
            metadata_refs,
        )
        self.assertIn(
            "ST005",
            {
                ...
                if issue.code == "ONSCREEN_CONTENT_UNIT_GAP"
                for source_id in issue.source_ids
            },
        )

    def test_empty_header_data_table_keeps_content_bearing_first_row_required(self) -> None:
        source_units = ({
            "unit_id": "SU-TABLE",
            "kind": "table",
            "locator": {"line_start": 1, "line_end": 4},
        ...
        records = {
            "ST001": {"statement": "课程A", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 3}},
            "ST002": {"statement": "课程A：交易申报实训", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 3}},
            "ST003": {"statement": "课程B", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 4}},
            "ST004": {"statement": "课程B：功率预测实训", "source_unit_refs": ["SU-TABLE"], "source_locator": {"line_start": 4}},
        }

        metadata_refs = _projected_table_header_record_ids(records, source_units)

        self.assertEqual(frozenset(), metadata_refs)
        self.assertNotIn("ST002", metadata_refs)
        self.assertNotIn("ST004", metadata_refs)

    def test_multicolumn_table_never_ignores_labelled_business_content(self) -> None:
        source_units = ({
            "unit_id": "SU-TABLE",
            "kind": "table",
            "text": "| 课程A | 交易申报实训 | 15万元/班 |",
        },)
        records = {
            "ST001": {"statement": "课程A", "source_unit_refs": ["SU-TABLE"]},
            "ST002": {"statement": "课程A：交易申报实训", "source_unit_refs": ["SU-TABLE"]},
            "ST003": {"statement": "课程A：15万元/班", "source_unit_refs": ["SU-TABLE"]},
        }
    ...
    def test_onscreen_content_unit_accepts_anchor_visible_in_page_title(self) -> None:
        page = replace(
            self._page("近期策略明确首期验证安排。"),
            title="核心结论",
            onscreen_text="近期策略",
        )
        issues = _page_content_unit_coverage_issues(
            ...
            {"content_units": [{
                ...
                "source_refs": ["ST008"],
                "full_prose_required": True,
                "coverage_anchors": ["近期策略"],
                "onscreen_required": True,
                "onscreen_anchors": ["近期策略", "核心结论"],
            }]},
        )
        self.assertNotIn("ONSCREEN_CONTENT_UNIT_GAP", {item.code for item in issues})

    def test_source_heading_anchor_does_not_force_hierarchy_ordinal_onscreen(self) -> None:
        page = replace(
            self._page("该空间用于说明业务具有持续培育价值。"),
            onscreen_text="长期判断：持续培育价值",
        )
        issues = _page_content_unit_coverage_issues(
            page,
            {"content_units": [{
                "unit_id": "p20-u05",
                "statement": "该空间用于说明业务具有持续培育价值。",
                "source_refs": ["ST0265"],
                "full_prose_required": True,
                "coverage_anchors": ["持续培育价值"],
                "onscreen_required": True,
                "onscreen_anchors": ["持续培育价值", "五、结论与提请研究事项"],
            }]},
        )
        self.assertNotIn("ONSCREEN_CONTENT_UNIT_GAP", {item.code for item in issues})
...
class OnscreenVisibilityContractTests(unittest.TestCase):
    def _page(self, evidence_map: str) -> ScriptPage:
        return ScriptPage(
            page_id="p06",
            sequence=6,
            heading="",
            page_type="content",
            title="首批客户与验证范围",
            main_message="建立客户池并验证真实机会。",
            full_prose="建立企业与院校客户池，并开展五项访谈。",
            selection_notes="",
            evidence_map=evidence_map,
            evidence_map_refs=("ST0063",),
            source_refs=("ST0063",),
            boundary_source_refs=(),
            boundary="",
            visual_structure="客户池与访谈信号共同筛选机会。",
            onscreen_text="【首轮验证】\n  首轮验证对象\n    企业客户池：约20家",
            module_titles=("首轮验证", "首轮验证对象", "企业客户池"),
        )

    @staticmethod
    def _contract(visibility: str) -> dict[str, object]:
        return {
            "page_consumption_contract_mode": "required",
            "content_units": [{
                "source_refs": ["ST0063"],
    ...
    def test_offscreen_fact_mapped_to_visible_module_is_rejected(self) -> None:
        issues = _onscreen_visibility_contract_issues(
            self._page("首轮验证对象→ST0063"),
            self._contract("prose_only"),
        )
    ...
    def test_offscreen_fact_mapped_to_declared_support_layer_is_allowed(self) -> None:
        for visibility, label in (
            ...
            with self.subTest(visibility=visibility):
                self.assertEqual(
                    [],
                    _onscreen_visibility_contract_issues(
                        self._page(f"{label}→ST0063"),
                        self._contract(visibility),
                    ),
                )

    def test_visible_fact_mapped_to_visible_module_is_allowed(self) -> None:
        self.assertEqual(
            [],
            _onscreen_visibility_contract_issues(
                self._page("首轮验证对象→ST0063"),
                self._contract("supporting_onscreen"),
            ),
        )
...
- 页面标题：正式材料
- 主判断：形成完整业务关系。
- 完整文字稿：正式材料完整说明需求变化、资源条件和运营机制三方面情况，并以明确事实和关系支撑页面判断，形成可独立理解的完整论述。需求方面，跨主体、跨区域协同需求持续增长，服务内容延伸至模型、分析和场景交付。资源方面，数据、模型和专业能力分属多方，使用范围和安全条件各不相同。运营方面，授权、交付、计量和结算相互贯通，运行结果持续返回产品和服务管理，形成闭环。
- 上屏文字：

需求变化
    协同范围：跨主体、跨区域协同需求持续增长扩大。
    服务内容：延伸至模型分析、场景交付等能力范围。

资源条件
    资源分布：数据、模型和专业能力分属多个主体。
    权利要求：使用范围和安全控制条件各不相同。

运营机制
    履约衔接：授权、交付、计量和结算相互贯通。
    持续改进：运行结果持续返回产品服务管理体系。
需求、资源和运营三方面共同支撑页面判断成立。
运行结果形成闭环反馈并持续优化改进机制。
- 证据：S001
...
"""
    ...
    def test_argument_function_contract_prevents_value_reference_gate_false_peer_warning(self) -> None:
        page = ScriptPage(
            page_id="p09", sequence=9, heading="", page_type="content",
            title="", main_message="", full_prose="", selection_notes="",
            evidence_map=(
                "long_term_value=【长期价值】→ST001；"
                "future_gate=【后续研究】→ST002"
            ), evidence_map_refs=("ST001", "ST002"), source_refs=(),
            boundary_source_refs=(), boundary="", visual_structure="",
            onscreen_text=(
                "【长期价值】\n"
                "  空间参考：0.5亿元/年\n"
            ...
            module_titles=("【长期价值】", "【后续研究】"),
        )
        contract = {
            "page_consumption_contract_mode": "required",
            "content_units": [
                {
                    "argument_function": "value_reference",
                    ...
                    "peer_set_id": "value_reference_set",
                },
                {
                    "argument_function": "gate_condition",
                    "relation_to_proposition": "gates",
                    "decision_scope": "future_gate",
                    "visibility": "supporting_onscreen",
                    "topology_role": "boundary",
                    "group_id": "future_gate",
                    "peer_set_id": "gate_set",
                },
            ]
        }
        ...
        self.assertNotIn(
            ...
            {issue.code for issue in _presentation_issues(page, contract)},
        )

    def test_required_contract_compares_only_modules_in_declared_peer_set(self) -> None:
        page = ScriptPage(
            page_id="p09", sequence=9, heading="", page_type="content",
            title="", main_message="", full_prose="", selection_notes="",
            evidence_map=(
                "product_a=产品甲→ST001；product_b=产品乙→ST002；"
                "gate=后续门槛→ST003"
            ), evidence_map_refs=("ST001", "ST002", "ST003"), source_refs=(),
            boundary_source_refs=(), boundary="", visual_structure="",
            onscreen_text=(
                "产品甲\n  内容：课程\n  用途：培训\n\n"
                "产品乙\n  内容形式：报告\n  支撑决策：经营\n\n"
            ...
            module_titles=("产品甲", "产品乙", "后续门槛"),
        )
        contract = {
            "page_consumption_contract_mode": "required",
            "content_units": [
                {
                    "visibility": "primary_onscreen", "group_id": "product_a",
                    "peer_set_id": "products", "decision_scope": "current_decision",
                    "relation_to_proposition": "supports",
                },
                {
                    "visibility": "primary_onscreen", "group_id": "product_b",
                    "peer_set_id": "products", "decision_scope": "current_decision",
                    "relation_to_proposition": "supports",
                },
                {
                    "visibility": "supporting_onscreen", "group_id": "gate",
                    "peer_set_id": "gates", "decision_scope": "future_gate",
                    "relation_to_proposition": "gates",
                },
            ],
        }
    ...
    def test_required_contract_detects_main_chain_order_mismatch(self) -> None:
        page = ScriptPage(
            page_id="p09", sequence=9, heading="", page_type="content",
            title="", main_message="", full_prose="", selection_notes="",
            evidence_map="step_a=第一步→ST001；step_b=第二步→ST002",
            evidence_map_refs=("ST001", "ST002"), source_refs=(),
            boundary_source_refs=(), boundary="", visual_structure="",
            onscreen_text="第二步\n  动作：形成报价\n\n第一步\n  动作：确认需求",
            module_titles=("第二步", "第一步"),
        )
        contract = {
            "page_consumption_contract_mode": "required",
            "content_units": [
                {
                    "visibility": "primary_onscreen", "group_id": "step_a",
                    "topology_role": "main_chain", "sequence_index": 1,
                    "decision_scope": "current", "relation_to_proposition": "supports",
                },
                {
                    "visibility": "primary_onscreen", "group_id": "step_b",
                    "topology_role": "main_chain", "sequence_index": 2,
                    "decision_scope": "current", "relation_to_proposition": "supports",
                },
            ],
        }
    ...
    def test_required_screen_groups_cannot_collapse_into_one_visible_module(self) -> None:
        page = ScriptPage(
            page_id="p09", sequence=9, heading="", page_type="content",
            title="", main_message="", full_prose="", selection_notes="",
            evidence_map=(
                "capabilities=【能力转化】→ST001；"
                "destinations=【能力转化】→ST002；"
                "reuse_relation=【能力转化】→ST003"
            ),
            evidence_map_refs=("ST001", "ST002", "ST003"), source_refs=(),
            boundary_source_refs=(), boundary="", visual_structure="",
            onscreen_text=(
        ...
        contract = {
            ...
            "content_units": [
                {
                    "unit_id": "p09-U01", "visibility": "primary_onscreen",
                    "group_id": "main_evidence", "onscreen_group_id": "capabilities",
                    "onscreen_group_kind": "taxonomy", "peer_dimension": "capability_type",
                    "topology_role": "satellite", "decision_scope": "current",
                    "relation_to_proposition": "supports",
                },
                {
                    "unit_id": "p09-U02", "visibility": "supporting_onscreen",
                    "group_id": "main_evidence", "onscreen_group_id": "destinations",
                    "onscreen_group_kind": "taxonomy", "peer_dimension": "delivery_destination",
                    "topology_role": "satellite", "decision_scope": "current",
                    "relation_to_proposition": "supports",
                },
                {
                    "unit_id": "p09-U03", "visibility": "supporting_onscreen",
                    "group_id": "main_evidence", "onscreen_group_id": "reuse_relation",
                    "onscreen_group_kind": "relation", "peer_dimension": "business_relation",
                    "topology_role": "satellite", "decision_scope": "current",
                    "relation_to_proposition": "explains",
                },
            ],
        }
        ...
        self.assertIn("ONSCREEN_GROUP_COALESCE_VIOLATION", codes)

    def test_required_screen_groups_allow_one_visible_module_per_declared_group(self) -> None:
        page = ScriptPage(
            page_id="p09", sequence=9, heading="", page_type="content",
            title="", main_message="", full_prose="", selection_notes="",
            evidence_map=(
                "capabilities=【专业能力】→ST001；"
                "destinations=【转化方向】→ST002；"
                "reuse_relation=【复用关系】→ST003"
            ),
            evidence_map_refs=("ST001", "ST002", "ST003"), source_refs=(),
            boundary_source_refs=(), boundary="", visual_structure="",
            onscreen_text=(
        ...
        self.assertNotIn("ONSCREEN_GROUP_COALESCE_VIOLATION", codes)
    ...
    def test_complete_business_sentences_under_named_modules_are_allowed(self) -> None:
        page = parse_script_markdown(
            "## 第1页：建设背景\n"
            "- 页面类型：内容页\n"
            "- 完整文字稿：三项并列现状构成建设背景。\n"
            "- 上屏文字：\n"
            "  **业务需求持续增长**\n"
            "    新型能源体系和新型电力系统加快建设，数字化、市场化和智能化持续提升，多源数据、专业知识、预测模型、持续监测和智能服务需求上升。\n"
            "  **行业资源较为分散**\n"
            "    数据、知识、模型、算法和专业能力分布于不同法人主体、业务系统和安全域，目录、口径、接口、标准、授权和版本尚未完全统一，协同成本较高。\n"
            "  **规模化服务机制仍需完善**\n"
            "    资源登记、产品封装、授权执行、可信交付、服务计量、客户评价和价值结算等环节尚待形成持续运营闭环。\n"
            "  三项现状共同构成数据服务与场景服务合作的建设背景。\n"
            "- 视觉结构：三项并列现状共同构成建设背景。\n"
        ).pages[0]
        ...
        self.assertNotIn("ONSCREEN_LINE_TOO_LONG", {issue.code for issue in issues})
        self.assertNotIn(
            "ONSCREEN_COMPLETE_PROPOSITION_UNGROUNDED",
            {issue.code for issue in issues},
        )
        self.assertTrue(_is_structured_compact_onscreen_layer(page))
    ...
    def test_complete_onscreen_proposition_over_ceiling_is_blocked(self) -> None:
        page = parse_script_markdown(
            ...
            "- 上屏文字：\n"
            + "平台以订单履行为主线，贯通客户购买、系统执行与合作伙伴结算，使各方结算依据保持一致。" * 3
            + "\n"
            "- 视觉结构：订单履行贯通服务执行与结算。\n"
        ).pages[0]
    ...
    def test_long_complete_sentence_without_a_module_or_landing_is_rejected(self) -> None:
        page = parse_script_markdown(
            "## 第1页：订单履行\n"
            "- 页面类型：内容页\n"
            "- 完整文字稿：订单履行贯通客户购买、系统执行与合作伙伴结算。\n"
        ...
        self.assertIn("ONSCREEN_COMPLETE_PROPOSITION_UNGROUNDED", codes)

    def test_integrated_landing_mapping_does_not_require_a_fake_module(self) -> None:
        page = parse_script_markdown(
            "## 第1页：建设背景\n"
            "- 页面类型：内容页\n"
            "- 上屏文字：\n"
            "  业务需求持续增长\n"
            "    多源数据、专业知识和持续监测服务需求上升。\n"
            "  三项现状共同构成资源连接、可信使用、产品组织和持续服务基础的建设背景。\n"
            "- 证据映射：\n"
            "  business_demand_current_state=业务需求持续增长→ST001\n"
            "  page_proposition=三项现状共同构成资源连接、可信使用、产品组织和持续服务基础的建设背景。→ST002\n"
        ).pages[0]
        contract = {
            "page_consumption_contract_mode": "required",
            "onscreen_structure_contract_mode": "required",
            "content_units": [
                {
                    "unit_id": "u1", "visibility": "supporting_onscreen",
                    "onscreen_group_id": "business_demand_current_state",
                    "onscreen_group_kind": "proposition",
                    "peer_dimension": "current_state",
                    "peer_set_id": "current_states",
                },
                {
                    "unit_id": "u2", "visibility": "supporting_onscreen",
                    "onscreen_group_id": "page_proposition",
                    "onscreen_group_kind": "proposition",
                    "peer_dimension": "page_judgment",
                },
            ],
            "page_logic_contract": {
                "onscreen_projection": [{
        ...
        codes = {
            issue.code
            for issue in _onscreen_module_dimension_consistency_issues(page, contract)
        }

        self.assertNotIn("ONSCREEN_GROUP_MODULE_MISSING", codes)
    ...
    def test_four_parallel_short_items_do_not_require_paragraph_density(self) -> None:
        page = parse_script_markdown(
            ...
            "- 页面类型：内容页\n"
            "- 完整文字稿："
            "合作对象覆盖电力及能源企业、科研院所及高校、数字科技企业、专业咨询机构和行业协会五类主体，各自依据资源条件和专业优势选择合作方式并明确责任。"
            "电力及能源企业提供真实行业场景与生产运行数据。科研院所及高校提供基础科研数据与知识模型。"
            "数字科技企业提供数字技术产品与治理工具支持。专业咨询机构提供行业研究成果与推广落地支持。"
            "行业协会提供行业协调与标准衔接支持。各方合作事项由专项协议明确范围、权利和责任边界。\n"
            "- 上屏文字：\n"
            "  **五类参与主体**\n"
            "      电力及能源企业：提供真实行业场景数据与生产运行数据。\n"
            "      科研院所及高校：提供基础科研数据与专业知识模型。\n"
            "      数字科技企业：提供数字技术产品与运行治理能力。\n"
            "      专业咨询机构：提供行业研究成果与落地推广支持。\n"
            "      行业协会：提供行业协调机制与标准衔接支持。\n"
            "具体范围、权利和责任边界由专项协议另行明确规定。\n"
            "合作方式可按实际业务需要动态调整完善优化。\n"
            "- 视觉结构：五类主体并列。\n"
        ).pages[0]
...
"""
    ...
    def test_expression_ir_allows_an_explicit_context_to_requirement_group(self) -> None:
        text = """业务需求持续增长
    新型能源体系和新型电力系统加快建设
    数字化、市场化和智能化持续提升
    对上述能力提出更高要求：多源数据、专业知识、预测模型、持续监测、智能服务"""
        contract = {
            "page_logic_contract": {
                "onscreen_expression": {
                    "nodes": [
                        {"id": "context", "render": "statement_stack", "items": ["新型能源体系和新型电力系统加快建设", "数字化、市场化和智能化持续提升"]},
                        {"id": "targets", "render": "chip_set", "items": ["多源数据", "专业知识", "预测模型", "持续监测", "智能服务"]},
                    ],
                    "edges": [{"from": "context", "to": "targets", "visible_label": "对上述能力提出更高要求"}],
                }
            }
        }

        self.assertEqual((), _onscreen_false_parallel_semantics(text, contract))
    ...
    def test_current_state_peers_require_contract_not_status_keywords(self) -> None:
        text = """建设背景
  业务需求持续增长
  行业资源较为分散
  规模化服务机制仍需完善"""
        self.assertTrue(_onscreen_false_parallel_semantics(text))

        contract = {
            "content_units": [
                {"peer_dimension": "current_state", "peer_set_id": "states", "onscreen_group_id": "demand"},
                {"peer_dimension": "current_state", "peer_set_id": "states", "onscreen_group_id": "resource"},
                {"peer_dimension": "current_state", "peer_set_id": "states", "onscreen_group_id": "service"},
            ],
            "page_logic_contract": {
                "onscreen_projection": [
                    {"carrier_mode": "integrated_proposition", "carrier": "业务需求持续增长"},
                    {"carrier_mode": "integrated_proposition", "carrier": "行业资源较为分散"},
                    {"carrier_mode": "integrated_proposition", "carrier": "规模化服务机制仍需完善"},
                ],
            },
        }
        self.assertEqual((), _onscreen_false_parallel_semantics(text, contract))
    ...
    def test_false_parallel_check_compares_direct_children_only(self) -> None:
        text = """业务结构
    能力基础
        行业建设加快
        现有供给不足
...
class ScriptContractAuditTests(unittest.TestCase):
    def test_required_fact_chain_gap_is_a_structured_script_error(self) -> None:
        outline = strict_outline({
            ...
            "source_refs": ["ST0063"],
            "page_consumption_contract_mode": "required",
            "argument_chain": [{"source_refs": ["ST0063"]}],
            "content_units": [{
                "unit_id": "p06-U01",
                "statement": "企业侧先建立约20家重点客户池。",
                "source_refs": ["ST0063"],
                "role": "supporting",
        ...
        script = """## 第6页：首批客户与验证范围
- 页面类型：内容页
- 页面标题：首批客户与验证范围
- 完整文字稿：企业侧先建立约20家重点客户池。
- 证据映射：完整稿支撑→ST0063
- 上屏文字：
  首轮验证范围
    先建立客户池
...
"""
        ...
        matching = [
            ...
            if issue.code == "ARGUMENT_CHAIN_FACT_VISIBILITY_MISSING"
        ]

        self.assertEqual(1, len(matching))
        self.assertEqual("error", matching[0].severity)
        self.assertEqual(("ST0063",), matching[0].source_ids)
        self.assertEqual(("step_index=1",), matching[0].evidence)
        self.assertIn("authoritative page plan", matching[0].suggested_action)

    def test_legacy_and_advisory_fact_chain_gaps_are_allowed(self) -> None:
        truth = source_truth({"id": "ST0063", "statement": "客户池。"})
        script = parse_script_markdown("""## 第6页：验证范围
- 页面类型：内容页
- 页面标题：验证范围
- 完整文字稿：建立客户池并验证机会。
- 证据映射：完整稿支撑→ST0063
        ...
        for mode in ("legacy", "advisory"):
            with self.subTest(mode=mode):
                outline = strict_outline({
                    ...
                    "title": "验证范围",
                    "source_refs": ["ST0063"],
                    "page_consumption_contract_mode": mode,
                    "argument_chain": [{"source_refs": ["ST0063"]}],
                    "content_units": [{
                        "source_refs": ["ST0063"],
                        "visibility": "prose_only",
                ...
                self.assertNotIn("ARGUMENT_CHAIN_FACT_VISIBILITY_MISSING", codes)

    def test_script_audit_blocks_offscreen_fact_promoted_by_evidence_map(self) -> None:
        outline = strict_outline({
            "page_id": "p06",
            "sequence": 6,
            "page_type": "content",
...
"""
    ...
    def test_script_audit_detects_all_p12_decorated_title_promotions(self) -> None:
        refs = ("ST0121", "ST0122", "ST0123")
        outline = strict_outline({
            "page_id": "p12",
            "sequence": 12,
            "page_type": "content",
            ...
            "content_units": [
                ...
                for index, (ref, statement, visibility) in enumerate((
                    ("ST0121", "第2—4个月开展首期试点。", "prose_only"),
                    ("ST0122", "第4—6个月开展复制验证。", "notes_only"),
                    ("ST0123", "验证通过后进入成熟放大。", "trace_only"),
                ), start=1)
            ],
        })
        truth = source_truth(
            {"id": "ST0121", "statement": "第2—4个月开展首期试点。"},
            {"id": "ST0122", "statement": "第4—6个月开展复制验证。"},
            {"id": "ST0123", "statement": "验证通过后进入成熟放大。"},
        )
        script = parse_script_markdown("""## 第12页：分阶段建设路径
- 页面类型：内容页
- 页面标题：分阶段建设路径
- 完整文字稿：首期试点后开展复制验证，验证通过后进入成熟放大。
- 证据映射：首期试点→ST0121；复制验证→ST0122；成熟放大→ST0123
- 上屏文字：
  ②首期试点｜第2—4个月
  ③复制验证｜第4—6个月
  ④成熟放大｜验证通过后
- 证据：ST0121、ST0122、ST0123
- 视觉结构：三个阶段按时间推进。
""")
        self.assertEqual(
            (
        ...
        matching = [
            issue
            for issue in issues
            if issue.code == "ONSCREEN_VISIBILITY_CONTRACT_BREACH"
        ]

        self.assertEqual(
            {("ST0121",), ("ST0122",), ("ST0123",)},
            {issue.source_ids for issue in matching},
        )
...
class RelationshipContinuityTests(unittest.TestCase):
    ...
    def test_current_page_blocks_reserved_course_composition_relation(self) -> None:
        page = self._page(
            8,
            mission="说明企业市场",
            core_message="企业班验证交易实训产品",
            onscreen="适合课程：交易与AI决策，集成两类预测模块",
        )
        contract = {
            "source_refs": ["ST0077"],
            "must_not_include": ["课程价格与模块组合由P10承担。"],
            "reserved_for_later_items": [
                {"topic": "课程与报价", "target_page": "P10"}
            ],
        }
        records = {
            "ST0077": {
        ...
        issues = _page_relationship_contract_issues(page, contract, records)

        self.assertIn("PAGE_SCOPE_RESERVED_RELATION", {item.code for item in issues})

    def test_main_audit_exposes_reserved_course_composition_relation(self) -> None:
        page = self._page(
            8,
            mission="说明企业市场",
            core_message="企业班验证交易实训产品",
        ...
        codes = {
            item.code
            for item in audit_script_quality(
                ScriptDocument((page,)),
                outline,
                truth,
            )
        ...
        self.assertIn("PAGE_SCOPE_RESERVED_RELATION", codes)

    def test_reserved_course_scope_allows_plain_course_list(self) -> None:
        page = self._page(
            8,
            mission="说明企业市场",
            core_message="企业班验证交易实训产品",
            onscreen="适合课程：交易与AI决策、负荷与电价预测、风光功率预测",
        )
        contract = {
            "source_refs": ["ST0077"],
            "must_not_include": ["课程价格与模块组合由P10承担。"],
        ...
        codes = {
            item.code
            for item in _page_relationship_contract_issues(page, contract, records)
        }

        self.assertNotIn("PAGE_SCOPE_RESERVED_RELATION", codes)
        self.assertNotIn("COMPOSITION_RELATION_UNSUPPORTED", codes)

    def test_ordinary_module_name_is_not_a_reserved_relation(self) -> None:
        page = self._page(
            8,
            mission="说明企业市场",
            core_message="企业班验证交易实训产品",
            onscreen="预测模块：覆盖数据处理、模型比较和偏差分析",
        )
        contract = {
            "source_refs": ["ST0077"],
            "must_not_include": ["课程价格与模块组合由P10承担。"],
        ...
        records = {
            "ST0077": {
                "statement": "预测课程覆盖数据处理、模型比较和偏差分析"
            }
        }
    ...
    def test_script_relation_accepts_p10_sourced_course_composition(self) -> None:
        page = self._page(
            10,
            mission="说明课程组合",
            core_message="预测模块并入交易课程",
            onscreen="负荷与电价预测、风光功率预测作为交易课程模块共同交付",
        )
        contract = {"source_refs": ["ST0118", "ST0123"]}
        records = {
            "ST0118": {
    ...
    def test_direct_entity_state_predicate_requires_review(self) -> None:
        page = self._page(
            8,
            mission="说明企业市场",
            core_message="企业岗位具备培训需求",
            onscreen="群体特征：交易、市场分析和预测岗位持续更新",
        )
        contract = {"source_refs": ["ST0075"]}
        records = {
            "ST0075": {
                "statement": "交易/市场分析/预测岗位：规则区域性强、结果可量化、需要持续更新；"
            }
        }

        issues = _page_relationship_contract_issues(page, contract, records)
        match = next(item for item in issues if item.code == "PREDICATE_OWNERSHIP_REVIEW")

        self.assertEqual("warning", match.severity)

    def test_attribute_owned_state_predicate_is_not_flagged(self) -> None:
        page = self._page(
            8,
            mission="说明企业市场",
            core_message="企业岗位具备培训需求",
            onscreen="群体特征：交易、市场分析和预测岗位的规则持续更新",
        )
        contract = {"source_refs": ["ST0075"]}
        records = {
            "ST0075": {
        ...
        self.assertNotIn("PREDICATE_OWNERSHIP_REVIEW", codes)
    ...
    def test_has_capability_relation_accepts_visible_business_wording(self) -> None:
        first = self._page(
            1,
            mission="说明能力基础",
            core_message="企业具备专业能力",
            onscreen="能力基础：企业具备电力专业能力",
            visual="企业拥有电力专业能力，支撑后续业务转化。",
            relations=[{"relation": "has_capability", "subject": "企业", "objects": ["专业能力"]}],
        )
    ...
    def test_uses_relation_accepts_visible_business_wording(self) -> None:
        first = self._page(
            1,
            mission="说明能力复用方式",
            core_message="培训业务复用专业能力",
            onscreen="能力复用：培训业务复用现有专业能力",
            visual="培训业务使用现有专业能力形成实训内容。",
            relations=[{"relation": "uses", "subject": "培训业务", "objects": ["专业能力"]}],
        )
    ...
    def test_responsible_for_relation_accepts_visible_business_wording(self) -> None:
        first = self._page(
            1,
            mission="说明主体职责",
            core_message="人才中心负责课程与客户",
            onscreen="主体职责：人才中心负责课程论证、客户访谈与培训组织",
            visual="人才中心牵头课程与客户工作。",
            relations=[
                {
    ...
    def test_operates_relation_accepts_visible_business_wording(self) -> None:
        first = self._page(
            1,
            mission="说明经营验证",
            core_message="联合项目组开展经营验证",
            onscreen="经营验证：联合项目组开展客户、合同与成本复盘",
            visual="联合项目组实施经营验证并管理成本台账。",
            relations=[
                {
                    "relation": "operates",
                    "subject": "联合项目组",
    ...
    def test_required_page_logic_contract_defers_to_its_own_carrier_check(self) -> None:
        # A page under page_logic_contract_mode="required" already has its
        # relation-to-screen visibility verified edge by edge by
        # cyberppt.page_logic_contract (ONSCREEN_RELATION_CARRIER_MISSING /
        # ONSCREEN_EXPRESSION_RELATION_MISSING). Re-running this module's
        # small fixed keyword table on the same page only produces
        # disagreements: a page that reads perfectly fine on screen but
        # phrases its connection with wording absent from the table would
        # otherwise be blocked for no real defect.
        first = self._page(
            1,
            mission="说明处理过程",
            core_message="数据经模型形成成果",
        ...
        codes = {
            ...
            for issue in self._issues(
                first, first_contract={"page_logic_contract_mode": "required"}
            )
        }
    ...
    def test_legacy_mode_page_is_still_checked_against_the_keyword_table(self) -> None:
        # The same authored content without a required page_logic_contract
        # keeps going through the legacy keyword check, so the deferral above
        # is scoped to pages that actually have the stronger contract.
        first = self._page(
            1,
            mission="说明处理过程",
            core_message="数据经模型形成成果",
    ...
    def test_generic_directed_relation_is_visible_through_its_own_arrow(self) -> None:
        # relation_semantics.py and stage02_relationship_adapter.py can only
        # type an unrecognized-but-explicitly-arrowed relation as the generic
        # "directed_relation" family. That family has no entry in this
        # module's keyword table, so it must fall back to treating the arrow
        # itself as the visible signal instead of being unconditionally
        # blocked regardless of what the page actually shows.
        first = self._page(
            1,
            mission="说明业务需求如何转化为要求",
            core_message="业务需求提出更高要求",
            onscreen="业务需求 → 提出更高要求",
            visual="以业务需求 → 提出更高要求呈现直接关系。",
            relations=[{"relation": "directed_relation", "subject": "业务需求", "objects": ["提出更高要求"]}],
        )
    ...
    def test_generic_directed_relation_without_any_arrow_still_blocks(self) -> None:
        # The fallback must not blanket-pass every generic relation: it only
        # recognizes the arrow itself, so a page that drops the arrow along
        # with any other visible connector is still caught.
        first = self._page(
            1,
            mission="说明业务需求如何转化为要求",
            core_message="业务需求提出更高要求",
