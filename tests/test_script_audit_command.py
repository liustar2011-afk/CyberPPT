...
- 文字稿取舍说明：
  ...
  - 仅讲解：职责分工与支撑机制。
  - 仅追溯：邻页目标框架与任务安排。
- 证据映射：公共能力定位→S015；职责分工与支撑机制→S026；会商与支撑机制→S059
- 上屏文字：
...
- 证据：S015、S026、S059
- 边界：正式范围经摸底验证后确定。
- 视觉结构：以行业公共能力为中心，连接服务对象、支撑机制、服务范围与专业职责分工；上屏文字由对应关系节点承载，不使用版式指令。
...
class ScriptAuditCommandTests(unittest.TestCase):
    ...
    def test_persists_passed_reports_and_ledger(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual("passed", report["quality_status"])
