...
首次内容适配与送图脚本编译前，主 Agent 在对话中提问“这套 PPT 主要用于哪种交流方式？”，
提供“演讲辅助（现场有人讲解，突出核心信息）”与“独立阅读（自行阅读，提供必要解释）”
两项选择，分别映射为 `presented` 与 `self_read`。两种类型均允许提炼、改写和图表表达。
等待回答后再进行依赖类型的适配与编译；已有用户明确选择时沿用，同批次续跑不重复询问。
自动默认的 `self_read` 不代表用户选择；兼顾两者时明确主要用途。
选择写入现有最终脚本页前的交流方式元数据；存在 JSON 镜像时同步 `deck.delivery_mode`
并通过现有渲染流程保持一致，不新增确认文件。具体执行规则见
[Stage 02 Skill 的交流类型提问](../.agents/skills/cyberppt-stage02-editable-pptx/SKILL.md#交流类型提问首次编译前)。
送图脚本审阅时核对实际交流方式区块；类型变化后重新编译与审阅，旧回执按输入身份失效。
...
待编写 SVG、待看图和失败均返回现有批次处理。主 Agent 按当前状态补齐资产或修复上游代码，再执行运行记录中的 `resume_command`。模板记录和交付记录共用同一命令生成器，正确引用含空格路径，保留生产分支、图片来源与提示词参数；普通续跑不继承强制重绘和跳过审计选项。禁止用临时导出脚本、直调底层 builder、修改最终包或合并单页成品代替正式入口。

清单准备阶段先在内存完成编译、旧产物复用校验和全量来源检查，通过后再发布提示词与清单。提示词或来源校验失败时保留原清单、检查点、提示词和图像。该保护覆盖校验失败，文件系统写入中断与跨进程并发另受现有文件锁及原子替换机制约束。

这些约束属于仓库工作流与防误用门禁，底层诊断和单元测试仍可独立执行。具有任意代码写入权限的进程可以修改门禁，因此不将进程内约束表述为安全隔离。正式交付必须来自完整编排和实际产物 QA。
...
Stage 02 默认直接读取 `references/visual-system.md` 中的 Style 09 正文。手动修改该文件后，重新组装生图脚本即可使用新内容；新项目也自动读取当前文件。

`--style-lock` 仅作为兼容输入，不要求人工确认、固定 Style 09、参考图哈希或不可变快照；读取时会刷新为当前 `references/visual-system.md` 内容。
...
用户要求生图后停下审阅时，正式命令增加 `--stop-after-images`，并保留生图、生产构建和组装参数。所有内容页图片完成尺寸归一化并通过文字审计后，返回 `paused_after_images`；原 manifest、build context 与运行汇总记录该状态，输出已审计图片路径。暂停发生在节奏检查、视觉来源绑定、清底和可编辑重建之前，三个组装分支均不会继续导出。失败图片继续按失败处理；只编译或 dry-run 不宣称已审计暂停。

用户看图并明确继续后，执行同一运行汇总中的 resume_command；该命令去掉暂停选项并保留原批次与生产参数。retry_command 保留暂停选项。不得自动越过用户要求的看图停点，也不新增审批文件。暂停选项不改变页面内容身份，恢复沿用有效图片回执。

每次开始生图或恢复生图前，主 Agent 必须向用户报告当前批次实际输出目录的绝对路径，
并提供可点击目录链接。目录以正式编排的 `artifacts.output_dir` / build_dir 为准，
图片位置以 manifest 的 `pairs[*].full.path` 为准。完成后再次报告目录和实际图片链接；
部分失败也须报告同一目录、成功图片及未完成页面。
