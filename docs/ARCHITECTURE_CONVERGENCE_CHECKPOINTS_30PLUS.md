...
- 阶段 38 workflow run `33351308499`：结论 `success`。Python 3.10/3.12 pytest、wheel build、repo 外 smoke 全绿。当前兼容层仍保留 6 个历史字段名，但生产模块全局 mutation 已缩减为 ImageGen 的 `run_codex_image` 与 `ensure_output_size` 两项。
