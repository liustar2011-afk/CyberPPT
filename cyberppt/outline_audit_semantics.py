...
def _semantic_derivation_issues(
    ...
    records = {
        str(record.get("id") or record.get("normalized_fact_id") or ""): record
        for field in ("records", "facts")
        for record in (source_truth or {}).get(field, [])
        if isinstance(record, dict)
        and (record.get("id") or record.get("normalized_fact_id"))
    }
    require_receipt = (
    ...
    for page in pages:
        ...
        for chain_index, node in enumerate(page.get("argument_chain") or [], start=1):
            if not isinstance(node, dict):
                continue
            statement = str(node.get("statement") or "").strip()
            evidence = node.get("evidence")
            normalized_refs = (
                evidence.get("normalized_fact_ids")
            ...
            refs = [
                ...
                for ref in (
                    normalized_refs
                    if isinstance(normalized_refs, list) and normalized_refs
                    else node.get("source_refs") or []
                )
                if str(ref)
            ]
            chain_evidence = source_text(refs, records)
            if not statement or not chain_evidence:
                continue
            for fidelity_issue in (
                *audit_semantic_strength(statement, chain_evidence),
                *(
                    audit_current_output_objects(statement, chain_evidence)
                    if str(node.get("role") or "").strip().lower()
                    in {"implementation", "output", "deliverable", "result"}
                    else []
                ),
            ):
                issues.append(AuditIssue(
                    fidelity_issue.code,
                    f"Argument chain item {chain_index}: {fidelity_issue.message}",
                    (page_id,),
...
def _page_content_unit_contract_issues(
    ...
    argument_functions = {
        "foundation", "decision_method", "execution_arrangement",
        "investment_boundary", "validation_output", "value_reference",
        "gate_condition", "operating_boundary", "deferred_constraint",
        "claim_basis", "mechanism", "action", "output", "metric",
        "example", "trace",
    }
    required = (
    ...
    for page in pages:
        ...
        for unit in units:
            ...
            argument_function = str(unit.get("argument_function") or "").strip()
            ...
            if argument_function and argument_function not in argument_functions:
                issues.append(AuditIssue(
                    "PAGE_CONTENT_UNIT_ARGUMENT_FUNCTION_INVALID",
                    "内容单元的 argument_function 必须使用已定义的论证功能，避免将价值参考、门槛条件和管理边界混为同一角色。",
                    (page_id,),
                    "classify_page_content_units",
                ))
        if onscreen_count == 0:
...
def _argument_chain_visibility_issues(
    ...
    """Keep required projected argument steps visible on their main chain."""

    issues: list[AuditIssue] = []
    for page in pages:
        if (
            page.get("page_type") != "content"
            or page.get("page_consumption_contract_mode") != "required"
        ):
            continue
        page_id = _page_id(page)
        records = [
        ...
        for gap in argument_chain_visibility_gaps(
            records,
            page.get("argument_chain"),
        ):
            refs = ", ".join(gap.source_refs)
            issues.append(AuditIssue(
                "ARGUMENT_CHAIN_FACT_VISIBILITY_MISSING",
                (
                    f"Argument-chain step {gap.step_index} cites facts [{refs}] "
                    "but none is primary_onscreen/supporting_onscreen with "
                    "topology_role=main_chain. Revise the authoritative page plan "
                    "and rerun the formal handoff."
                ),
                (page_id,),
                "restore_argument_chain_fact_visibility",
            ))
    return issues
...
def _structural_argument_duty_issues(
    ...
    for page in pages:
        if page.get("page_type") != "content":
            continue
        # Required page-consumption contracts already carry page-scoped duties
        # in content_units, and _page_content_unit_contract_issues audits those
        # duties against their authored visibility. Reapplying the global
        # Source Truth duty here would override the approved page hierarchy.
        if page.get("page_consumption_contract_mode") == "required":
            continue
        page_id = str(page.get("page_id") or "")
