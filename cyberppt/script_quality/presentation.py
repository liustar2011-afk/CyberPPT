...
from .onscreen import (
    ...
    _complete_proposition_placement_issues,
    _declared_count,
    _has_any,
    ...
    _legacy_onscreen_structure_migration_issues,
...
def _presentation_issues(
    ...
    *,
    ...
    if page.page_type == "content":
        ...
        false_parallel_hits = _onscreen_false_parallel_semantics(
            page.onscreen_text, contract,
        )
        ...
        ungrounded_propositions = _complete_proposition_placement_issues(
            page, contract,
        )
        if ungrounded_propositions:
            issues.append(
                _issue(
                    "ONSCREEN_COMPLETE_PROPOSITION_UNGROUNDED",
                    page,
                    "A long complete on-screen sentence has no business module or declared page-logic landing.",
                    "Place the sentence beneath the business module it elaborates, or declare it as the source-grounded integrated landing in the authoritative page logic contract.",
                    evidence=ungrounded_propositions[:3],
                )
            )
        ...
        declared_landings = {
            str(node.get("id") or ""): re.sub(r"\s+", "", str(node.get("text") or "")).strip()
            for node in expression.get("nodes") or []
            if isinstance(node, dict) and str(node.get("render") or "") == "landing"
        } if isinstance(expression, dict) else {}
        if declared_landings:
            landing_labels = {
                node_id: re.sub(r"\s+", "", str(edge.get("visible_label") or "")).strip()
                for edge in expression.get("edges") or []
                if isinstance(edge, dict) and str(edge.get("to") or "") in declared_landings
                for node_id in [str(edge.get("to") or "")]
            }

            def is_declared_landing(line: str) -> bool:
                compact_line = re.sub(r"\s+", "", line).strip()
                return any(
                    landing and landing in compact_line
                    and (not landing_labels.get(node_id) or landing_labels[node_id] in compact_line)
                    for node_id, landing in declared_landings.items()
                )

            detail_phrase_overages = [
                (line, chars)
                for line, chars in detail_phrase_overages
                if not is_declared_landing(line)
            ]
    ...
    loop_like = not expression_ir_present and ("闭环" in visual or "闭环回流" in visual)
    ...
    if page.page_type == "content" and page.top_level_module_titles and not expression_ir_present:
    ...
    long_lines = tuple(
        line for line, _chars in _onscreen_detail_phrase_overages(page.onscreen_text)
    )
    if long_lines:
        issues.append(
            _issue(
                ...
                "Keep a single complete proposition when its subject, state, and relation must remain together; split only paragraph-like copy into source-grounded business modules.",
    ...
    if page.page_type == "content" and not expression_ir_present:
