...
def _logic_projection_covers_unit(
    contract: dict[str, object], source_refs: tuple[str, ...], onscreen_surface: str,
) -> bool:
    """Accept the author-declared logic compression for a visible source unit.

    A required page-logic contract supplies the more precise preservation
    invariant: source anchors may be compressed, but the node's declared
    decisive signals must remain visible.  This avoids forcing the older
    literal-anchor rule to turn the screen back into a full-prose duplicate.
    """

    if page_logic_mode(contract) != "required":
        return False
    compact_surface = re.sub(r"\s+", "", onscreen_surface)
    for node in (contract.get("page_logic_contract") or {}).get("nodes") or []:
        if not isinstance(node, dict):
            continue
        if not set(str(ref) for ref in node.get("source_refs") or []).intersection(source_refs):
            continue
        if str(node.get("onscreen_requirement") or "required") != "required":
            continue
        signals = [str(value).strip() for value in node.get("onscreen_signals") or [] if str(value).strip()]
        if signals and all(re.sub(r"\s+", "", signal) in compact_surface for signal in signals):
            return True
    return False
...
def _projected_table_header_record_ids(
    records_by_id: dict[str, dict[str, object]],
    source_units: tuple[dict[str, object], ...] = (),
) -> frozenset[str]:
    """Identify table-header records only from confirmed row coordinates.

    Empty Markdown headers make the first body row ambiguous.  Wording,
    shortness and repeated ``标签：内容`` projections cannot resolve that
    ambiguity.  A source unit must therefore carry a confirmed header row
    with exact table coordinates and cells; the projected record must match
    both that source line and its mechanically reconstructed statement.
    """

    confirmed_by_unit: dict[str, tuple[int, frozenset[str]]] = {}
    for unit in source_units:
        if str(unit.get("kind") or "") != "table":
            continue
        ...
        table_header = (
            ...
            and isinstance(metadata.get("table_header"), dict)
            else None
        )
        locator = unit.get("locator")
        if (
            not unit_id
            or not isinstance(table_header, dict)
            or table_header.get("status") != "confirmed"
            or not isinstance(locator, dict)
        ):
            continue
        row_index = table_header.get("row_index")
        line_start = table_header.get("line_start")
        table_start = locator.get("line_start")
        cells = table_header.get("cells")
        if (
            not isinstance(row_index, int)
            or row_index < 1
            or not isinstance(line_start, int)
            or not isinstance(table_start, int)
            or line_start != table_start + row_index + 1
        ...
        statements = {
            (
                cell.strip()
                if cell_index == 1 or not row_label
                else f"{row_label}：{cell.strip()}"
            )
            for cell_index, cell in enumerate(cells, start=1)
            if cell.strip()
        }
        confirmed_by_unit[unit_id] = (line_start, frozenset(statements))

    result: set[str] = set()
    for record_id, record in records_by_id.items():
        statement = str(record.get("statement") or "").strip()
        source_locator = record.get("source_locator")
        if not statement or not isinstance(source_locator, dict):
            continue
        record_line = source_locator.get("line_start")
        for value in record.get("source_unit_refs") or []:
            confirmed = confirmed_by_unit.get(str(value))
            if (
                confirmed is not None
                and record_line == confirmed[0]
                and statement in confirmed[1]
            ):
...
def _full_prose_source_coverage_issues(
    ...
    contract: dict[str, object],
    records_by_id: dict[str, dict[str, object]],
    structural_metadata_refs: frozenset[str] = frozenset(),
) -> list[ScriptQualityIssue]:
    ...
    for ref in expected_refs:
        if ref in omissions or ref in structural_metadata_refs:
            continue
...
def _full_prose_paragraph_boundary_issues(
    ...
    expected_refs = tuple(
        dict.fromkeys(
            ...
            if (
                str(ref).strip()
                and str(ref) not in detail_refs
                and str(ref) not in structural_metadata_refs
                and ref in records_by_id
            )
        )
    )
    ...
    mapped_refs = tuple(
        ref
        for refs, _ in mapping
        for ref in refs
        if ref not in structural_metadata_refs
    )
    ...
    for index, (refs, reason) in enumerate(mapping):
        ...
        if index >= len(paragraphs):
            continue
        for ref in refs:
            if ref in structural_metadata_refs:
                continue
            record = records_by_id.get(ref)
            if not record:
                continue
...
def _page_content_unit_coverage_issues(
    ...
    for unit in units:
        ...
        if source_refs and set(source_refs).issubset(structural_metadata_refs):
            continue
        ...
        if (
            ...
            # 标题、副标题与上屏文字共同构成读者可见层。页面标题可能直接
            # 承载来源表格或小节的短锚点（例如“核心结论”）；副标题则按
            # page-script-contract.md 承载核心判断。已经由这两层表达的锚点
            # 不应被迫在正文重复，因此三处一起检索。
            onscreen_surface = "\n".join(
                (page.title, page.subtitle, page.onscreen_text)
            )
            if _logic_projection_covers_unit(contract, source_refs, onscreen_surface):
                continue
            missing = tuple(
                anchor
                for anchor in onscreen_anchors
                if not _is_onscreen_metadata_unit(anchor)
