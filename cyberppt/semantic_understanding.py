...
- A source-unit assignment may not rely on `summary` alone. Add `atomic_items`; each item must contain `item_id`, a source-faithful `statement`, `source_unit_refs`, `status`, and at least two source-specific `coverage_anchors`. Preserve named business objects, actors, actions, processing targets, conditions, states and numeric facts in the item; use optional machine-readable `actors`, `conditions`, and `numeric_facts` arrays when present. If one source unit contains independently removable drivers, consequences, gaps, responses, different statuses, or metadata (cover titles, subtitles, author/organization lines, dates, catalog labels), split it into multiple atomic items so each keeps one coherent claim. Every assigned source unit must occur in at least one atomic item. An item's evidence role, priority, and argument duty in Source Truth are derived automatically from its target semantic node's `argument_role`/`argument_weight` — do not redeclare them per item.
