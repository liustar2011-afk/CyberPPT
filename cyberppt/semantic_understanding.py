...
- Set root `source_truth_projection_mode` to `required`. A source-unit assignment may not rely on `summary` alone. Add `atomic_items`; each item must contain `item_id`, a source-faithful `statement`, `source_unit_refs`, semantic `claim_role`, Source Truth `evidence_role` (`fact`, `change`, `problem`, `judgment`, `recommendation`, `boundary`, or `unresolved`), Source Truth `evidence_priority` (`P0`, `P1`, or `P2`), `argument_duty` (`premise`, `driver`, `consequence`, `gap`, `response`, `support`, `detail`, `boundary`, or `metadata`), `importance`, `status`, `claim_origin`, and at least two source-specific `coverage_anchors`. Use `metadata` for cover titles, subtitles, author or organization lines, dates, catalog labels, and other document-administration text: preserve them in Source Truth for traceability, but never assign them to a content page's prose or on-screen evidence. Preserve named business objects, actors, actions, processing targets, conditions, states and numeric facts in the item; use optional machine-readable `actors`, `conditions`, and `numeric_facts` arrays when present. If one source unit contains independently removable drivers, consequences, gaps, responses, different evidence roles, semantic roles, priorities, statuses, or metadata, split it into multiple atomic items. Every assigned source unit must occur in at least one atomic item. A structural premise/driver/consequence/gap/response may not use `evidence_priority=P2`; do not classify it as a background example merely because it appears before a compact conclusion.
