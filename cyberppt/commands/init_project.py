...
5. New formal solution projects use `argument_contract_mode: strict`, `editorial_control_mode: required`, and `cyberppt.outline.v2`. Build each page from source-grounded `core_message`, `content_units`, and `content_relations`; page types and claim taxonomies do not determine the page meaning. Preserve all source evidence, but never give it equal page weight: P0 is page-forming, P1 supports grouped modules, and P2 remains in `detail_refs` for prose, notes, parameters, or traceability. Each content page must also declare one concrete `audience_question`, a non-empty `must_not_include` list, and `split_risk`; medium/high risks require reasons and high risk blocks approval until resolved.
