...
def prepare_outline_input(project: Path) -> Path:
    ...
    lines += [
        ...
        "- `source_argument_node_roles`: required object; copy each selected node's `argument_role` exactly; an `advantage` node must not be represented as a `foundation` node",
