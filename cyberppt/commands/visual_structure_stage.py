...
def _build_executable_page(source: dict[str, Any], decision: dict[str, Any]) -> dict[str, Any]:
    ...
    if content_nodes:
        for key in evidence_keys:
            ...
            if len(ids) < 2:
                continue
            root_ids = {str(content_nodes[tid].get("root_id")) for tid in ids if tid in content_nodes}
            if len(root_ids) > 1:
                _fail(
    ...
    for item in source.get("business_relationships") or []:
        ...
        relationship = {
            ...
            for key in (
                "subject", "relation", "objects", "direction", "condition",
                "modality", "basis", "confidence", "source_refs", "authority_ref",
            )
            if key in item
        }
...
def run_visual_structure_audit(project: Path, script: Path) -> tuple[int, dict[str, Any]]:
    ...
    review_summary.write_text(_render_visual_review_summary(audited_spec, _read_json(decisions), {"deck_rhythm": results["deck_rhythm"]}), encoding="utf-8", newline="\n")
