...
def _semantic(page: Mapping[str, object]) -> tuple[str, str, float]:
    ...
    return (
        str(topology.get("primary_topology") or "unknown"),
...
def audit_deck_structure_collapse(pages: Sequence[Mapping[str, object]]) -> dict[str, object]:
    ...
    Repetition alone is never treated as proof of an error: a deck may
    legitimately contain several peer pages.  Blocking is reserved for a
    semantic mismatch or for repeated visual parallelism that contradicts at
    least one page's semantic topology.
    """

    content = [page for page in pages if isinstance(page, Mapping) and str(page.get("page_role") or "") != "chapter"]
    ...
    for page in content:
        ...
        if semantic in _DIRECTED_SEMANTIC_TOPOLOGIES and visual in _PEER_LIKE_VISUAL_TOPOLOGIES:
            issue = {
                ...
                "page_ids": [page_id],
                "message": f"{page_id} has verified semantic topology {semantic!r} but selected visual topology {visual!r}.",
            }
    ...
    if total >= 5 and common_form_ratio > 0.35:
        warnings.append({
            ...
            "page_ids": [],
            "message": f"Expression form {common_form!r} appears on {common_form_ratio:.0%} of content pages; review for structural overuse.",
        })
    if total >= 5 and peer_like_ratio > 0.40:
        warnings.append({
            "code": "DECK_PEER_LIKE_TOPOLOGY_CONCENTRATION",
            "page_ids": [],
            "message": f"Peer-like visual topology appears on {peer_like_ratio:.0%} of content pages; review whether semantics were flattened.",
        })
    if total >= 5 and common_visual_ratio > 0.50:
        warnings.append({
            "code": "DECK_VISUAL_TOPOLOGY_CONCENTRATION",
            "page_ids": [],
            "message": f"Visual topology {common_visual!r} appears on {common_visual_ratio:.0%} of content pages.",
        })
        ...
        for index in range(0, max(0, len(rows) - length + 1)):
            ...
            if length == 3:
                ...
                blocking.append({
                    ...
                    "page_ids": page_ids,
                    "message": "Four consecutive pages use parallel_set while at least one page is not semantically peer_set.",
                })
            else:
                warnings.append({
                    "code": "DECK_TOPOLOGY_RUN_WARNING",
                    "page_ids": page_ids,
                    "message": f"Four consecutive content pages use visual topology {visual!r}; verify this repetition is semantically justified.",
                })
    ...
    return {
        ...
        "metrics": {
            ...
            "same_semantic_topology": {"topology": common_semantic, "ratio": round(common_semantic_ratio, 4)},
            "same_visual_topology": {"topology": common_visual, "ratio": round(common_visual_ratio, 4)},
            "same_expression_form": {"form": common_form, "ratio": round(common_form_ratio, 4)},
