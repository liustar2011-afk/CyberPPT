...
def _labels(relationships: Sequence[Mapping[str, object]]) -> str:
    return " ".join(_text(item.get("relation_label")) for item in relationships if isinstance(item, Mapping))


def _relation_names(relationships: Sequence[Mapping[str, object]]) -> set[str]:
    return {_text(item.get("relation")) for item in relationships if isinstance(item, Mapping) and _text(item.get("relation"))}
...
def resolve_relation_expression(*, relationships: Sequence[Mapping[str, object]], module_count: int) -> tuple[str, tuple[str, ...]] | None:
    ...
    if "optional_progression" in names:
        return "parallel_classification_3_6", ("semantic:optional_progression", *evidence)
    if any(token in labels for token in _FEEDBACK_LABELS) or names & {"feedback", "feeds_back", "feeds_back_to", "returns_to", "iterates", "loops_to"}:
    ...
    if "comparison" in names:
        return ("comparison_2col", ("semantic:comparison", *evidence)) if module_count == 2 else ("mapping_2_6", ("semantic:multi_item_comparison", *evidence))
